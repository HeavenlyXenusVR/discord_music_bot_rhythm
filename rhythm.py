        ...
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    await cur.execute(
                        "SELECT position_seconds FROM rhythm_playback_state WHERE guild_id = %s AND bot_name = 'rhythm' AND (is_playing = TRUE OR is_paused = TRUE OR position_seconds > 0) ORDER BY is_playing DESC, is_paused DESC, position_seconds DESC LIMIT 1",
                        (target_guild_id,),
                    )
