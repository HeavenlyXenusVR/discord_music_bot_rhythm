    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute("SELECT id, video_url, title FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT %s", (interaction.guild.id, index-1))
                rows = await cur.fetchall()
                for r in rows:
                    await cur.execute("DELETE FROM rhythm_queue WHERE id = %s AND guild_id = %s AND bot_name = 'rhythm'", (r[0], interaction.guild.id))
                    await cur.execute(
                        "DELETE FROM rhythm_queue_backup WHERE video_url = %s AND title = %s AND guild_id = %s AND bot_name = 'rhythm' LIMIT 1",
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                for url, title in q:
                    await cur.execute("INSERT INTO rhythm_user_playlists (user_id, playlist_name, video_url, title) VALUES (%s, %s, %s, %s)", (interaction.user.id, name, url, title))
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                res = await cur.fetchone()
                if res and res[0] != 'none': return await interaction.response.send_message(embed=discord.Embed(description="❌ **Conflict:** Disable standard Filters via `/rhythm_main_filter none` first.", color=discord.Color.red()), ephemeral=True)
...
async def ensure_swarm_command_tables():
    global _last_swarm_bridge_db_error_log_at, swarm_command_tables_ready, swarm_command_tables_retry_after
    if swarm_command_tables_ready:
        return True
    now = time.time()
    if now < swarm_command_tables_retry_after:
        return False
    async with swarm_command_tables_lock:
        if swarm_command_tables_ready:
            return True
        if time.time() < swarm_command_tables_retry_after:
            return False
        try:
            async with DBPoolManager() as pool:
                async with pool.acquire() as conn:
                    async with conn.cursor() as cur:
                        await cur.execute("CREATE TABLE IF NOT EXISTS rhythm_swarm_overrides (guild_id BIGINT, bot_name VARCHAR(50), command VARCHAR(20), attempts INT NOT NULL DEFAULT 0, last_error TEXT NULL, PRIMARY KEY(guild_id, bot_name))")
                        await cur.execute("CREATE TABLE IF NOT EXISTS rhythm_swarm_direct_orders (id INT AUTO_INCREMENT PRIMARY KEY, bot_name VARCHAR(50), guild_id BIGINT, vc_id BIGINT NULL, text_channel_id BIGINT NULL, command VARCHAR(50), data TEXT NULL, attempts INT NOT NULL DEFAULT 0, last_error TEXT NULL)")
                        for stmt in [
