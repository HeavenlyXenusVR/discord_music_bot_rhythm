...
async def build_autodj_query(cur, guild_id):
    await cur.execute("SELECT title FROM rhythm_history WHERE guild_id = %s ORDER BY played_at DESC LIMIT 12", (guild_id,))
...
async def maybe_enqueue_autodj(cur, guild, channel_id):
    ...
    try:
        ...
        history_titles = set()
        await cur.execute("SELECT title FROM rhythm_history WHERE guild_id = %s ORDER BY played_at DESC LIMIT 18", (guild.id,))
