...
async def aria_command_listener():
    try:
        ...
        for row in commands:
            ...
            if cmd == 'RESTART':
                await cur.execute("DELETE FROM rhythm_swarm_overrides WHERE guild_id = %s AND bot_name = %s", (guild_id, 'rhythm'))
            ...
            if guild:
                ...
                if executed or attempts + 1 >= DIRECT_ORDER_MAX_ATTEMPTS or not guild:
                    ...
                    await cur.execute("UPDATE rhythm_swarm_overrides SET attempts = COALESCE(attempts, 0) + 1, last_error = %s WHERE guild_id = %s AND bot_name = %s", (f"state_mismatch:{cmd}", guild_id, 'rhythm'))
    except Exception as tx_error:
