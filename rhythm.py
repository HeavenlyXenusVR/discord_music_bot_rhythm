                ...
                if vc:
                    ...
                    if state:
                        schedule_named_task(f"persistent_voice_restore_process_queue:{guild.id}", process_queue(guild, channel_id, start_position=state.get("position", 0), allow_recovery_restore=True))
                                ...
                                await cur.execute("UPDATE rhythm_voice_state SET reconnect_attempts = reconnect_attempts + 1, last_error = %s WHERE guild_id = %s AND bot_name = 'rhythm'", ("rejoin_failed", guild.id))
