...
DB_CONFIG = {
    ...
    'db': os.getenv(f"{BOT_ENV_PREFIX}_DB_NAME", "discord_music_rhythm"),
    'autocommit': True
}
...
async def init_db():
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("CREATE TABLE IF NOT EXISTS rhythm_playback_state (guild_id BIGINT, bot_name VARCHAR(50), channel_id BIGINT, video_url TEXT, position_seconds INT DEFAULT 0, is_playing BOOLEAN DEFAULT FALSE, title TEXT, PRIMARY KEY (guild_id, bot_name))")
                ...
                await cur.execute("CREATE TABLE IF NOT EXISTS rhythm_swarm_direct_orders (id INT AUTO_INCREMENT PRIMARY KEY, bot_name VARCHAR(50), guild_id BIGINT, vc_id BIGINT, text_channel_id BIGINT, command VARCHAR(50), data TEXT)")
...
async def restore_active_playback_entry(cur, guild_id, requester_id=None):
    await cur.execute(
        "SELECT channel_id, video_url, title, position_seconds FROM rhythm_playback_state WHERE guild_id = %s AND bot_name = %s AND is_playing = TRUE LIMIT 1",
        (guild_id, 'rhythm')
    )
    row = await cur.fetchone()
    if not row:
    ...
    await cur.execute(
        "SELECT id, video_url FROM rhythm_queue WHERE guild_id = %s AND bot_name = %s ORDER BY id ASC LIMIT 1",
        (guild_id, 'rhythm')
    )
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                if not next_song:
                    await cur.execute("UPDATE rhythm_playback_state SET is_playing = FALSE WHERE guild_id = %s AND bot_name = 'rhythm'", (guild.id,))
                ...
                await cur.execute("DELETE FROM rhythm_queue WHERE id = %s", (song_id,))
...
async def bootstrap_recovery_from_storage():
    ...
    try:
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    await cur.execute("SELECT guild_id, channel_id, position_seconds FROM rhythm_playback_state WHERE is_playing = TRUE AND bot_name = %s", ('rhythm',))
                    rows = await cur.fetchall()
    except Exception as exc:
            ...
            async with conn.cursor() as cur:
                for guild_id, data in list(playback_tracking.items()):
                    ...
                    if guild and guild.voice_client and (getattr(guild.voice_client, 'playing', False) or getattr(guild.voice_client, 'paused', False)):
                        ...
                        await cur.execute("REPLACE INTO rhythm_playback_state (guild_id, bot_name, channel_id, video_url, position_seconds, is_playing, title) VALUES (%s, 'rhythm', %s, %s, %s, %s, %s)", (guild_id, data['channel_id'], data['url'], pos, is_playing, data['title']))
...
@bot.tree.command(name="rhythm_main_setfeedback", description="Choose the text channel where this bot posts now-playing updates, queue actions, and recovery notices.")
@commands.has_permissions(administrator=True)
...
# --- PLAYBACK COMMANDS ---
@bot.tree.command(name="rhythm_main_play", description="Queue a track, URL, livestream, search result, or supported playlist and start playback if the bot is idle.")
...
async def clear(interaction: discord.Interaction):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("DELETE FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm'", (interaction.guild.id,))
                await cur.execute("UPDATE rhythm_playback_state SET is_playing = FALSE, position_seconds = 0 WHERE guild_id = %s AND bot_name = 'rhythm'", (interaction.guild.id,))
    await bot.change_presence(status=discord.Status.online)
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                row = await cur.fetchone()
                if row: 
                    await cur.execute("DELETE FROM rhythm_queue WHERE id = %s", (row[0],))
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                rows = await cur.fetchall()
                for r in rows: await cur.execute("DELETE FROM rhythm_queue WHERE id = %s", (r[0],))
                        ...
                        async with conn.cursor() as cur:
                            for entry in new_tracks:
                                ...
                                if t_url:
                                    ...
                                    added_count += 1
                            await cur.execute("UPDATE rhythm_active_playlists SET known_track_count = %s WHERE guild_id = %s", (current_count, guild_id))
...
async def aria_command_listener():
    try:
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    await cur.execute("SELECT guild_id, command FROM rhythm_swarm_overrides WHERE bot_name = %s", ('rhythm',))
        ...
        for row in commands:
            ...
            cmd = row['command']
            if cmd == 'RESTART':
                logger.warning('[rhythm] Restart command received from swarm override listener. Restarting process...')
                async with DBPoolManager() as pool:
                    async with pool.acquire() as conn:
                        async with conn.cursor() as cur:
                            await cur.execute("DELETE FROM rhythm_swarm_overrides WHERE bot_name = %s", ('rhythm',))
                await bot.close()
                sys.exit(0)
            guild = bot.get_guild(guild_id)
...
async def direct_order_listener():
    try:
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    await cur.execute("SELECT * FROM rhythm_swarm_direct_orders WHERE bot_name = %s", ('rhythm',))
                ...
                if guild.voice_client and getattr(guild.voice_client, 'playing', False):
                    ...
                    if not track_title:
                        async with DBPoolManager() as pool:
                            async with pool.acquire() as conn:
                                async with conn.cursor(aiomysql.DictCursor) as cur:
                                    await cur.execute("SELECT title FROM rhythm_playback_state WHERE guild_id = %s AND bot_name = 'rhythm' AND is_playing = TRUE LIMIT 1", (guild.id,))
                                    res = await cur.fetchone()
