...
def get_process_queue_lock(guild_id):
    return _redis_client.lock(f"rhythm:process_queue_lock:{guild_id}", timeout=REDIS_LOCK_TIMEOUT_SECONDS)

def get_track_requeue_lock(guild_id):
    return _redis_client.lock(f"rhythm:track_requeue_lock:{guild_id}", timeout=REDIS_LOCK_TIMEOUT_SECONDS)

def get_voice_connect_lock(guild_id):
    return _redis_client.lock(f"rhythm:voice_connect_lock:{guild_id}", timeout=REDIS_LOCK_TIMEOUT_SECONDS)
...
async def _do_guild_setting_update(interaction, *, column, value=None, toggle=False):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                if toggle:
                    await cur.execute(f"SELECT {column} FROM rhythm_guild_settings WHERE guild_id = %s", (interaction.guild.id,))
                    res = await cur.fetchone()
                    value = not res[0] if res else True
                await cur.execute(f"UPDATE rhythm_guild_settings SET {column} = %s WHERE guild_id = %s", (value, interaction.guild.id))
    invalidate_feature_caches(interaction.guild.id)
    return value
                        ...
                        async with DBPoolManager() as pool:
                            async with pool.acquire() as conn:
                                async with conn.cursor() as cur:
                                    ...
                                    await cur.execute("UPDATE rhythm_active_playlists SET known_track_count = %s WHERE guild_id = %s AND bot_name = 'rhythm'", (len(effective_rows), guild_id))
                        break
