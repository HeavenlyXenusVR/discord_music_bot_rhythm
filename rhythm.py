...
DB_CONFIG = {
    'host': os.getenv(f"{BOT_ENV_PREFIX}_DB_HOST") or os.getenv("DB_HOST") or os.getenv("MYSQL_HOST") or "127.0.0.1",
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute("CREATE TABLE IF NOT EXISTS rhythm_queue (id INT AUTO_INCREMENT PRIMARY KEY, guild_id BIGINT, bot_name VARCHAR(50), video_url TEXT, title TEXT, requester_id BIGINT DEFAULT NULL)")
                await cur.execute("CREATE TABLE IF NOT EXISTS rhythm_queue_backup (id INT AUTO_INCREMENT PRIMARY KEY, guild_id BIGINT, bot_name VARCHAR(50), video_url TEXT, title TEXT, requester_id BIGINT DEFAULT NULL)")
                ...
                await cur.execute("CREATE TABLE IF NOT EXISTS rhythm_swarm_overrides (guild_id BIGINT, bot_name VARCHAR(50), command VARCHAR(20), attempts INT NOT NULL DEFAULT 0, last_error TEXT NULL, PRIMARY KEY(guild_id, bot_name))")
...
async def get_current_track_snapshot(guild_id):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute(
                    "SELECT video_url, title FROM rhythm_playback_state WHERE guild_id = %s AND bot_name = 'rhythm' AND (is_playing = TRUE OR is_paused = TRUE) ORDER BY is_playing DESC LIMIT 1",
                    (guild_id,),
                )
                row = await cur.fetchone()
    if not row:
            ...
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    if not actual_connected:
                        ...
                        await cur.execute(
                            "UPDATE rhythm_playback_state SET channel_id = COALESCE(%s, channel_id), is_playing = %s, is_paused = %s WHERE guild_id = %s AND bot_name = 'rhythm'",
                            (actual_channel_id, actual_playing, actual_paused, guild.id),
                        )
    except Exception:
                    ...
                    for g in guilds:
                        ...
                        await cur.execute(
                            ...
                            (
                                ...
                                int(metrics_row.get("position_seconds") or 0),
...
async def snapshot_queue_backup(cur, guild_id):
    await cur.execute("SELECT video_url, title, requester_id FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC", (guild_id,))
    ...
    if insert_data:
        await cur.executemany(
            "INSERT INTO rhythm_queue_backup (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, %s, %s, %s, %s)",
            insert_data,
        )
...
async def backup_track(cur, guild_id, video_url, title, requester_id):
    await cur.execute(
        "SELECT COUNT(*) FROM rhythm_queue_backup WHERE guild_id = %s AND bot_name = 'rhythm' AND video_url = %s AND title = %s",
    ...
    if existing and int(_scalar_from_row(existing, 0) or 0) > 0:
        return 0
    await cur.execute(
        "INSERT INTO rhythm_queue_backup (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, 'rhythm', %s, %s, %s)",
...
async def enqueue_track(cur, guild_id, video_url, title, requester_id, *, backup=True):
    await cur.execute(
        "INSERT INTO rhythm_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, 'rhythm', %s, %s, %s)",
        (guild_id, video_url, title, requester_id),
    )
    if backup:
        try:
...
async def delete_live_queue_copies(cur, guild_id, video_url, title):
    ...
    if not target_key:
        return 0
    await cur.execute(
        "SELECT id, video_url, title FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm'",
        (guild_id,),
    )
    rows = list(await cur.fetchall() or [])
    deleted = 0
...
async def shuffle_queue_rows(cur, guild_id, *, preserve_first=True):
    await cur.execute("SELECT id, guild_id, bot_name, video_url, title, requester_id FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC", (guild_id,))
    rows = list(await cur.fetchall() or [])
    if len(rows) <= 1:
        return len(rows)
    head = []
    ...
    try:
        ...
        if insert_data:
            await cur.executemany(
                "INSERT INTO rhythm_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, %s, %s, %s, %s)",
                insert_data,
            )
...
async def restore_queue_from_backup(cur, guild_id, requester_id=None):
    ...
    await cur.execute("SELECT video_url, title, requester_id FROM rhythm_queue_backup WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC", (guild_id,))
    rows = list(await cur.fetchall() or [])
    if not rows:
        return 0
    ...
    for row in rows:
        ...
        insert_data.append((guild_id, 'rhythm', url, title, requester_id if requester_id is not None else backup_requester_id))
    if insert_data:
        await cur.executemany(
            "INSERT INTO rhythm_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, %s, %s, %s, %s)",
            insert_data,
...
async def repair_queue_backup_parity(cur, guild_id, *, reason="queue_integrity", active_player=None):
    ...
    """
    await cur.execute("SELECT id, video_url, title, requester_id FROM rhythm_queue_backup WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC", (guild_id,))
    backup_rows = list(await cur.fetchall() or [])
    await cur.execute("SELECT id, video_url, title, requester_id FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC", (guild_id,))
    live_rows = list(await cur.fetchall() or [])
    if not backup_rows and not live_rows:
        return 0, 0

    await cur.execute(
        "SELECT video_url, title, is_playing, is_paused, position_seconds FROM rhythm_playback_state WHERE guild_id = %s AND bot_name = 'rhythm' LIMIT 1",
        (guild_id,),
    )
...
async def restore_active_playback_entry(cur, guild_id, requester_id=None):
    await cur.execute(
        "SELECT video_url, title FROM rhythm_playback_state "
    ...
    await cur.execute(
        "SELECT video_url, title FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT 25",
        (guild_id,),
    )
...
async def requeue_failed_track(cur, guild_id, channel_id, url, title, requester_id, *, position=0, reason="recovery"):
    ...
    async with get_track_requeue_lock(guild_id):
        ...
        try:
            await cur.execute(
                "SELECT video_url, title, requester_id FROM rhythm_queue_backup WHERE guild_id = %s AND bot_name = 'rhythm' AND (video_url = %s OR title = %s) ORDER BY id ASC LIMIT 1",
                (guild_id, url, title),
            )
        ...
        if recent_until and now < recent_until:
            await cur.execute(
                "SELECT video_url, title FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT 12",
                (guild_id,),
            )
            existing_rows = list(await cur.fetchall() or [])
            already_queued = any(
            ...
            await insert_queue_front(cur, "rhythm_queue", guild_id, "rhythm", url, title, requester_id)
            await remember_recovery_state(guild_id, channel_id, position)
        ...
        await cur.execute(
            "SELECT video_url, title FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT 8",
            (guild_id,),
        )
        existing_rows = list(await cur.fetchall() or [])
        already_queued = any(
                    ...
                    async with conn.cursor() as cur:
                        ...
                        if reason == "FINISHED":
                            ...
                            if loop_mode == 'queue':
                                ...
                                await insert_queue_front(cur, "rhythm_queue", guild.id, "rhythm", track_uri, track_title, original_requester)
                            else:
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute("SELECT id, video_url, title, requester_id FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT 1", (guild.id,))
                ...
                if not next_song:
                    await cur.execute(
                        "UPDATE rhythm_playback_state SET channel_id = NULL, video_url = NULL, title = NULL, position_seconds = 0, is_playing = FALSE, is_paused = FALSE WHERE guild_id = %s AND bot_name = 'rhythm'",
                        (guild.id,),
                    )
                ...
                try:
                    ...
                    if _is_direct_media_url(url):
                        playback_tracking.pop(guild.id, None)
                        await cur.execute(
                            "UPDATE rhythm_playback_state SET video_url = NULL, title = NULL, is_playing = FALSE, is_paused = FALSE, position_seconds = 0 WHERE guild_id = %s AND bot_name = 'rhythm'",
                            (guild.id,),
                        )
...
async def stop_playback(guild):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("UPDATE rhythm_playback_state SET channel_id = NULL, video_url = NULL, title = NULL, position_seconds = 0, is_playing = FALSE, is_paused = FALSE WHERE guild_id = %s AND bot_name = 'rhythm'", (guild.id,))
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                if queue_rows:
                    await cur.executemany(
                        "INSERT INTO rhythm_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, %s, %s, %s, %s)",
                        [(interaction.guild.id, 'rhythm', url, title, requester_id) for url, title, requester_id in queue_rows],
                    )
                    try:
...
async def playnext(interaction: discord.Interaction, search: str):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT id, video_url, title, requester_id FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC", (interaction.guild.id,))
                existing_rows = await cur.fetchall()
                insert_data = [(interaction.guild.id, 'rhythm', track.uri, track.title, interaction.user.id)]
                insert_data.extend((
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute("UPDATE rhythm_playback_state SET channel_id = NULL, video_url = NULL, title = NULL, position_seconds = 0, is_playing = FALSE, is_paused = FALSE WHERE guild_id = %s AND bot_name = 'rhythm'", (interaction.guild.id,))
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute("SELECT id, video_url, title FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT 1 OFFSET %s", (interaction.guild.id, index-1))
                row = await cur.fetchone()
                if row:
                    await cur.execute("DELETE FROM rhythm_queue WHERE id = %s AND guild_id = %s AND bot_name = 'rhythm'", (row[0], interaction.guild.id))
                    await cur.execute(
                        "DELETE FROM rhythm_queue_backup WHERE video_url = %s AND title = %s AND guild_id = %s AND bot_name = 'rhythm' LIMIT 1",
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute("SELECT video_url, title FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT %s", (interaction.guild.id, skip_count))
                rows = list(await cur.fetchall() or [])
                if rows:
                    try:
                        await cur.execute("START TRANSACTION")
...
async def bump(interaction: discord.Interaction, index: int):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT id, video_url, title, requester_id FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT 1 OFFSET %s", (interaction.guild.id, index-1))
                row = await cur.fetchone()
                if not row:
                    return await interaction.response.send_message("Invalid index.", ephemeral=True)
                await cur.execute("DELETE FROM rhythm_queue WHERE id = %s AND guild_id = %s AND bot_name = 'rhythm'", (row[0], interaction.guild.id))
                await insert_queue_front(cur, "rhythm_queue", interaction.guild.id, "rhythm", row[1], row[2], row[3])
                await snapshot_queue_backup(cur, interaction.guild.id)
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                if snapshot_rows:
                    ...
                    await cur.executemany(
                        "INSERT INTO rhythm_active_playlist_tracks (guild_id, bot_name, playlist_url, position_idx, track_key, video_url, title, requester_id) VALUES (%s, 'rhythm', %s, %s, %s, %s, %s, %s)",
    ...
    for (guild_id, url, known_count, req_id, channel_id), data in zip(playlists, results):
        try:
            if isinstance(data, Exception):
                logger.error("[rhythm] Playlist sync extraction failed for guild %s: %s", guild_id, data)
                continue
            ...
            async with DBPoolManager() as pool:
                async with pool.acquire() as conn:
                    async with conn.cursor() as cur:
                        ...
                        await cur.execute("SELECT track_key, video_url, title, requester_id FROM rhythm_active_playlist_tracks WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY position_idx ASC", (guild_id,))
                        ...
                        purged_backup = 0
                        if removed_counts:
                            await cur.execute("SELECT id, video_url, title FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC", (guild_id,))
                            ...
                            await cur.execute("SELECT id, video_url, title FROM rhythm_queue_backup WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC", (guild_id,))
                        ...
                        if added_rows:
                            await cur.executemany(
                                "INSERT INTO rhythm_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, %s, %s, %s, %s)",
                                [(guild_id, 'rhythm', t_url, t_title, t_req) for t_url, t_title, t_req, _key in added_rows],
                            )
                            try:
            ...
            logger.error("[rhythm] Playlist sync loop failed for guild %s: %s", guild_id, e, exc_info=True)
...
class SwarmIntelligence(commands.Cog):
    ...
    async def heartbeat(self):
        try:
            ...
            logger.debug("[%s] Heartbeat update skipped.", self.bot_name, exc_info=True)
...
async def queue_integrity_check_loop():
    ...
    from rhythm_queue_backup so playback can resume cleanly.
    """
    try:
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    ...
                    for row in candidates:
                        ...
                        await cur.execute(
                            "SELECT video_url, title, requester_id FROM rhythm_queue_backup "
                            "WHERE guild_id = %s AND bot_name = 'rhythm' "
