    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await dedupe_queue_table_by_track_key(cur, "rhythm_queue", bot_name="rhythm")
                await dedupe_queue_table_by_track_key(cur, "rhythm_queue_backup", bot_name="rhythm")
...
async def collect_and_persist_metrics(guild=None):
    ...
    async def _fetch_guild_metrics_row(pool, g, voice_connected, player_playing, player_paused, live_position):
        ...
        try:
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    await cur.execute(
                        ...
                        LEFT JOIN rhythm_playback_state ps
                          ON ps.guild_id = seed.guild_id AND ps.bot_name = 'rhythm'
                        LIMIT 1
                        """,
                        (g.id, g.id, g.id),
                    )
            ...
            logger.debug("[rhythm] Per-guild metrics SELECT failed for guild %s", g.id, exc_info=True)
            return {}
                ...
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    ...
                    for (g, channel_id, voice_connected, player_playing, player_paused, live_position, live_duration), metrics_row in zip(guild_snapshots, fetch_results):
                        ...
                        if not voice_connected and (metrics_row.get("is_playing") or metrics_row.get("is_paused")):
                            await cur.execute("UPDATE rhythm_playback_state SET is_playing = FALSE, is_paused = FALSE WHERE guild_id = %s AND bot_name = 'rhythm'", (g.id,))
...
async def restore_queue_from_backup(cur, guild_id, requester_id=None):
    ...
    removed_content_dupes = await dedupe_queue_table_by_track_key(cur, "rhythm_queue_backup", guild_id=guild_id, bot_name="rhythm")
