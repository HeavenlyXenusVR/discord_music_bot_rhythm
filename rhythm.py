...
async def build_autodj_query(cur, guild_id):
    await cur.execute("SELECT title FROM rhythm_history WHERE guild_id = %s ORDER BY played_at DESC LIMIT 8", (guild_id,))
...
async def maybe_enqueue_autodj(cur, guild, channel_id):
    ...
    try:
        ...
        history_titles = set()
        await cur.execute("SELECT title FROM rhythm_history WHERE guild_id = %s ORDER BY played_at DESC LIMIT 12", (guild.id,))
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute("CREATE TABLE IF NOT EXISTS rhythm_swarm_direct_orders (id INT AUTO_INCREMENT PRIMARY KEY, bot_name VARCHAR(50), guild_id BIGINT, vc_id BIGINT, text_channel_id BIGINT, command VARCHAR(50), data TEXT, attempts INT NOT NULL DEFAULT 0, last_error TEXT NULL)")
...
async def ensure_swarm_command_tables():
    try:
        ...
        logger.exception("Failed ensuring swarm command tables for rhythm.")
...
async def direct_order_listener():
    try:
        ...
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    await cur.execute("SELECT id, bot_name, guild_id, vc_id, text_channel_id, command, data, COALESCE(attempts, 0) AS attempts FROM rhythm_swarm_direct_orders WHERE bot_name = %s ORDER BY id ASC", ('rhythm',))
                ...
                async with pool.acquire() as conn:
                    async with conn.cursor() as cur:
                        if executed or attempts >= 2 or not guild:
                            ...
                            await cur.execute("UPDATE rhythm_swarm_direct_orders SET attempts = COALESCE(attempts, 0) + 1, last_error = %s WHERE id = %s", (f"unexecuted:{cmd}", oid))
