                ...
                async with DBPoolManager() as pool:
                    async with pool.acquire() as conn:
                        async with conn.cursor() as cur:
                            ...
                            if reason == "FINISHED":
                                if loop_mode == 'queue':
                                    await cur.execute("INSERT INTO rhythm_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, 'rhythm', %s, %s, %s)", (payload.player.guild.id, payload.track.uri, payload.track.title, bot.user.id if bot.user else None))
                                elif loop_mode == 'song':
                                    await cur.execute("SELECT COALESCE(MIN(id), 0) FROM rhythm_queue WHERE guild_id = %s", (payload.player.guild.id,))
                                    min_row = await cur.fetchone()
                                    new_id = ((min_row[0] if min_row else 0) or 0) - 1
                                    await cur.execute("INSERT INTO rhythm_queue (id, guild_id, bot_name, video_url, title, requester_id) VALUES (%s, %s, 'rhythm', %s, %s, %s)", (new_id, payload.player.guild.id, payload.track.uri, payload.track.title, bot.user.id if bot.user else None))
        except Exception:
            pass
                ...
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    ...
                    for orphan in orphans:
                        ...
                        if guild:
                            ...
                            await cur.execute("SELECT MIN(id) as m_id FROM rhythm_queue WHERE guild_id = %s", (guild.id,))
                            min_res = await cur.fetchone()
                            new_id = (min_res['m_id'] or 0) - 1
                            await cur.execute("INSERT INTO rhythm_queue (id, guild_id, bot_name, video_url, title, requester_id) VALUES (%s, %s, 'rhythm', %s, %s, %s)", (new_id, guild.id, orphan['video_url'], orphan.get('title', 'Resumed Track'), bot.user.id))
...
async def init_playlist_db():
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute('''
                    CREATE TABLE IF NOT EXISTS rhythm_active_playlists (guild_id BIGINT, bot_name VARCHAR(50), playlist_url TEXT, known_track_count INT DEFAULT 0, requester_id BIGINT, PRIMARY KEY (guild_id, bot_name))
                ''')
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute("SELECT guild_id, playlist_url, known_track_count, requester_id FROM rhythm_active_playlists WHERE bot_name = 'rhythm'")
                    ...
                    async with pool.acquire() as conn:
                        async with conn.cursor() as cur:
                            for entry in new_tracks:
                                ...
                                if t_url: await cur.execute("INSERT INTO rhythm_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, 'rhythm', %s, %s, %s)", (guild_id, t_url, t_title, req_id))
                    ...
                    if guild.id in playback_tracking:
                        ...
                        if now - track_info.get('start_time', 0) > 10:
                            ...
                            async with DBPoolManager() as pool:
                                async with pool.acquire() as conn:
                                    async with conn.cursor() as cur:
                                        await cur.execute("SELECT id FROM rhythm_queue WHERE guild_id = %s LIMIT 1", (guild.id,))
