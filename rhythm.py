...
async def delete_live_queue_copies(cur, guild_id, video_url, title):
    await cur.execute(
        "DELETE FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' AND (video_url = %s OR title = %s)",
        (guild_id, video_url, title),
    )
...
async def restore_active_playback_entry(cur, guild_id, requester_id=None):
    ...
    title = _row_value(row, "title", _row_value(row, 1))
    await cur.execute(
        "SELECT video_url FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT 1",
        (guild_id,),
    )
    existing = await cur.fetchone()
        ...
        try:
            ...
            if payload.track:
                async with DBPoolManager() as pool:
                    async with pool.acquire() as conn:
                        async with conn.cursor() as cur:
                            await cur.execute("SELECT loop_mode FROM rhythm_guild_settings WHERE guild_id = %s", (payload.player.guild.id,))
                            ...
                            if reason == "FINISHED":
                                ...
                                if loop_mode == 'queue':
                                    ...
                                    await insert_queue_front(cur, "rhythm_queue", payload.player.guild.id, "rhythm", payload.track.uri, payload.track.title, original_requester)
                                else:
        ...
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    await cur.execute(
                        "SELECT position_seconds FROM rhythm_playback_state WHERE guild_id = %s AND bot_name = 'rhythm' AND is_playing = TRUE LIMIT 1",
                        (target_guild_id,),
                    )
...
async def on_wavelink_node_ready(payload: wavelink.NodeReadyEventPayload):
    ...
    try:
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    await cur.execute("SELECT guild_id, channel_id, position_seconds, video_url, title FROM rhythm_playback_state WHERE is_playing = TRUE AND bot_name = 'rhythm'")
...
async def filter_cmd(interaction: discord.Interaction, mode: str):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                if mode != 'none': await cur.execute("UPDATE rhythm_guild_settings SET filter_mode = %s, custom_modifiers_left = 0 WHERE guild_id = %s", (mode, interaction.guild.id))
    ...
    class AdvancedPanel(discord.ui.View):
        ...
        async def shuf(self, i: discord.Interaction, b: discord.ui.Button):
            ...
            async with DBPoolManager() as pool:
                async with pool.acquire() as conn:
                    async with conn.cursor() as cur:
                        await cur.execute("SELECT id, video_url, title, requester_id FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm'", (i.guild.id,))
                        q = await cur.fetchall()
                        if not q: return await i.followup.send("Queue empty.")
                        l = list(q); random.shuffle(l)
                        await cur.execute("DELETE FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm'", (i.guild.id,))
                        for row in l:
                    ...
                    async with pool.acquire() as conn:
                        async with conn.cursor() as cur:
                            ...
                            if queue_rows:
                                await cur.executemany(
                                    "INSERT INTO rhythm_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, %s, %s, %s, %s)",
                                    [(guild_id, 'rhythm', t_url, t_title, req_id) for t_url, t_title, req_id in queue_rows],
                                )
                                try:
