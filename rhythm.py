            ...
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    for g in guilds:
                        ...
                        await cur.execute("SELECT COUNT(*) AS total FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm'", (g.id,))
                        queue_row = await cur.fetchone() or {}
                        await cur.execute("SELECT COUNT(*) AS total FROM rhythm_queue_backup WHERE guild_id = %s AND bot_name = 'rhythm'", (g.id,))
                        backup_row = await cur.fetchone() or {}
                        await cur.execute("SELECT is_playing, is_paused, position_seconds FROM rhythm_playback_state WHERE guild_id = %s AND bot_name = 'rhythm' LIMIT 1", (g.id,))
                        ...
                        await cur.execute(
                            ...
                            (
                                ...
                                int(playback_row.get("position_seconds") or 0),
