    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                try: await cur.execute("ALTER TABLE rhythm_playback_state ADD COLUMN title TEXT")
                except: pass
                try: await cur.execute("ALTER TABLE rhythm_playback_state ADD COLUMN is_paused BOOLEAN DEFAULT FALSE")
                except: pass
                ...
                try: await cur.execute("ALTER TABLE rhythm_guild_settings MODIFY loop_mode VARCHAR(10) DEFAULT 'queue'")
                except: pass
                try: await cur.execute("ALTER TABLE rhythm_guild_settings ADD COLUMN fade_seconds FLOAT DEFAULT 5.0")
                except: pass
                try: await cur.execute("ALTER TABLE rhythm_guild_settings ADD COLUMN fade_curve VARCHAR(20) DEFAULT 'linear'")
                except: pass
                try: await cur.execute("UPDATE rhythm_guild_settings SET loop_mode = 'queue' WHERE loop_mode IS NULL OR loop_mode NOT IN ('off', 'song', 'queue')")
                except: pass
                ...
                try: await cur.execute("ALTER TABLE rhythm_queue ADD COLUMN bot_name VARCHAR(50)")
                except: pass
                try: await cur.execute("ALTER TABLE rhythm_queue_backup ADD COLUMN bot_name VARCHAR(50)")
                except: pass
                try: await cur.execute("ALTER TABLE rhythm_queue ADD COLUMN requester_id BIGINT DEFAULT NULL")
                except: pass
                try: await cur.execute("ALTER TABLE rhythm_queue_backup ADD COLUMN requester_id BIGINT DEFAULT NULL")
                except: pass
                try: await cur.execute("ALTER TABLE rhythm_history ADD COLUMN requester_id BIGINT DEFAULT NULL")
                except: pass
                ...
                try: await cur.execute("CREATE INDEX rhythm_track_intelligence_recent_idx ON rhythm_track_intelligence (guild_id, last_played)")
                except: pass
                try: await cur.execute("CREATE INDEX rhythm_track_intelligence_requester_idx ON rhythm_track_intelligence (guild_id, last_requester_id, last_played)")
                except: pass
                try: await cur.execute("CREATE INDEX rhythm_user_affinity_recent_idx ON rhythm_user_track_affinity (guild_id, user_id, last_requested)")
                except: pass
                try: await cur.execute("CREATE INDEX rhythm_smart_recommendations_recent_idx ON rhythm_smart_recommendations (guild_id, created_at)")
                except: pass
                ...
                try: await cur.execute("ALTER TABLE rhythm_voice_state ADD COLUMN reconnect_attempts INT NOT NULL DEFAULT 0")
                except: pass
                try: await cur.execute("ALTER TABLE rhythm_voice_state ADD COLUMN last_error TEXT NULL")
                except: pass
                try: await cur.execute("CREATE INDEX rhythm_voice_state_rejoin_idx ON rhythm_voice_state (bot_name, desired_connected, last_seen_at)")
                except: pass
                try: await cur.execute("CREATE INDEX rhythm_metrics_status_idx ON rhythm_metrics (bot_name, updated_at)")
                except: pass
                ...
                try: await cur.execute("ALTER TABLE rhythm_active_playlists ADD COLUMN channel_id BIGINT DEFAULT NULL")
                except: pass
                ...
                try: await cur.execute("CREATE INDEX rhythm_queue_lookup_idx ON rhythm_queue (guild_id, bot_name, id)")
                except: pass
                try: await cur.execute("CREATE INDEX rhythm_queue_backup_lookup_idx ON rhythm_queue_backup (guild_id, bot_name, id)")
                except: pass
                try: await cur.execute("CREATE INDEX rhythm_history_guild_played_idx ON rhythm_history (guild_id, played_at)")
                except: pass
                try: await cur.execute("CREATE INDEX rhythm_history_requester_idx ON rhythm_history (guild_id, requester_id, played_at)")
                except: pass
                try: await cur.execute("CREATE INDEX rhythm_playback_resume_idx ON rhythm_playback_state (bot_name, is_playing, guild_id)")
                except: pass
                try: await cur.execute("CREATE INDEX rhythm_playlist_bot_idx ON rhythm_active_playlists (bot_name, guild_id)")
                except: pass
                try: await cur.execute("CREATE INDEX rhythm_user_playlist_lookup_idx ON rhythm_user_playlists (user_id, playlist_name)")
                except: pass
                try: await cur.execute("ALTER TABLE rhythm_playback_state ADD COLUMN bot_name VARCHAR(50) DEFAULT 'rhythm'")
                except: pass
                try: await cur.execute("ALTER TABLE rhythm_active_playlists ADD COLUMN bot_name VARCHAR(50) DEFAULT 'rhythm'")
                except: pass
                try: await cur.execute("ALTER TABLE rhythm_bot_home_channels ADD COLUMN bot_name VARCHAR(50) DEFAULT 'rhythm'")
                except: pass
...
@bot.tree.command(name="rhythm_main_shuffle", description="Randomize the order of the upcoming queue while keeping the currently playing track untouched.")
async def shuffle(interaction: discord.Interaction):
    if not await is_dj(interaction): return
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT id, video_url, title, requester_id FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm'", (interaction.guild.id,))
