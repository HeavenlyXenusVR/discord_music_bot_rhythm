                ...
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    for g in guilds:
                        ...
                        await cur.execute(
                            ...
                            VALUES (%s, 'rhythm', %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, 0, %s, %s)
                            """,
                            (
                                g.id,
                                voice_connected,
...
async def restore_active_playback_entry(cur, guild_id, requester_id=None):
    ...
    await cur.execute(
        "SELECT video_url, title, track_uid FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT 25",
        (guild_id,),
    )
...
async def requeue_failed_track(cur, guild_id, channel_id, url, title, requester_id, *, position=0, reason="recovery", track_uid=None):
    ...
    async with get_track_requeue_lock(guild_id):
        ...
        if recent_until and now < recent_until:
            await cur.execute(
                "SELECT video_url, title, track_uid FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT 12",
                (guild_id,),
            )
            existing_rows = list(await cur.fetchall() or [])
            already_queued = any(
        ...
        await cur.execute(
            "SELECT video_url, title, track_uid FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT 8",
            (guild_id,),
        )
        existing_rows = list(await cur.fetchall() or [])
        already_queued = any(
                    ...
                    async with conn.cursor() as cur:
                        ...
                        if reason == "FINISHED":
                            ...
                            if loop_mode == 'queue':
                                ...
                                await insert_queue_front(cur, "rhythm_queue", guild.id, "rhythm", track_uri, track_title, original_requester, track_data.get("track_uid"))
                            else:
                ...
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    ...
                    try:
                        ...
                        await cur.execute(
                            ...
                            FOR UPDATE
                            """,
                            ('rhythm', DIRECT_ORDER_MAX_ATTEMPTS, DIRECT_ORDER_CLAIM_TIMEOUT_SECONDS, DIRECT_ORDER_FETCH_LIMIT),
                        )
                        ...
                        if ids:
                            placeholders = ','.join(['%s'] * len(ids))
                            await cur.execute(
                                f"""UPDATE rhythm_swarm_direct_orders
                                    SET claimed_at = NOW(), claim_token = %s
                                    WHERE id IN ({placeholders})
                                      AND bot_name = %s
                                      AND COALESCE(attempts, 0) < %s
                                      AND (claimed_at IS NULL OR claimed_at <= NOW() - INTERVAL %s SECOND)""",
                                (DIRECT_ORDER_CLAIM_TOKEN, *ids, 'rhythm', DIRECT_ORDER_MAX_ATTEMPTS, DIRECT_ORDER_CLAIM_TIMEOUT_SECONDS),
                            )
                            await cur.execute(
                                """
                ...
                async with pool.acquire() as conn:
                    async with conn.cursor() as cur:
                        try:
                            await cur.execute("START TRANSACTION")
                            if executed or attempts + 1 >= DIRECT_ORDER_MAX_ATTEMPTS or not guild:
                                await cur.execute("DELETE FROM rhythm_swarm_direct_orders WHERE id = %s", (oid,))
                            else:
                                await cur.execute("UPDATE rhythm_swarm_direct_orders SET attempts = COALESCE(attempts, 0) + 1, last_error = %s, claimed_at = DATE_SUB(NOW(), INTERVAL %s SECOND), claim_token = NULL WHERE id = %s", (f"unexecuted:{cmd}", DIRECT_ORDER_RETRY_BACKDATE_SECONDS, oid))
                            await cur.execute("COMMIT")
                        except Exception:
                            try:
                                await cur.execute("ROLLBACK")
...
class SwarmIntelligence(commands.Cog):
    ...
    async def heartbeat(self):
        try:
            ...
            logger.exception("[rhythm] Heartbeat update failed.")
