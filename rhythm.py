                ...
                async with conn.cursor() as cur:
                    for guild_id, data in list(playback_tracking.items()):
                        try:
                            ...
                            await cur.execute(
                                "UPDATE rhythm_playback_state SET position_seconds = %s, is_playing = %s, is_paused = %s WHERE guild_id = %s AND bot_name = %s",
...
async def init_db():
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("CREATE TABLE IF NOT EXISTS rhythm_playback_state (guild_id BIGINT, bot_name VARCHAR(50), channel_id BIGINT, video_url TEXT, position_seconds INT DEFAULT 0, is_playing BOOLEAN DEFAULT FALSE, is_paused BOOLEAN DEFAULT FALSE, title TEXT, PRIMARY KEY (guild_id, bot_name))")
            ...
            async with conn.cursor() as cur:
                for guild_id, data in list(playback_tracking.items()):
                    ...
                    if guild and guild.voice_client and _player_is_playing(guild.voice_client):
                        ...
                        await cur.execute("REPLACE INTO rhythm_playback_state (guild_id, bot_name, channel_id, video_url, position_seconds, is_playing, is_paused, title) VALUES (%s, 'rhythm', %s, %s, %s, TRUE, FALSE, %s)", (guild_id, data['channel_id'], data['url'], pos, data['title']))
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                else: await cur.execute("UPDATE rhythm_guild_settings SET filter_mode = %s WHERE guild_id = %s", (mode, interaction.guild.id))
    if interaction.guild.voice_client:
        wav_filters = wavelink.Filters()
