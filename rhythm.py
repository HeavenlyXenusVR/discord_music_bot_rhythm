...
async def _process_queue_inner(guild, channel_id, start_position=0, *, allow_recovery_restore=False):
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT volume, loop_mode, filter_mode, transition_mode, custom_speed, custom_pitch, custom_modifiers_left, stay_in_vc, fade_seconds, fade_curve FROM rhythm_guild_settings WHERE guild_id = %s", (guild.id,))
                res = await cur.fetchone()
