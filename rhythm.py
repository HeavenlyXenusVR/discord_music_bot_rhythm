                ...
                async with DBPoolManager() as pool:
                    async with pool.acquire() as conn:
                        async with conn.cursor() as cur:
                            ...
                            if reason == "FINISHED":
                                if loop_mode == 'queue':
                                    await cur.execute("INSERT INTO rhythm_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, 'rhythm', %s, %s, %s)", (payload.player.guild.id, payload.track.uri, payload.track.title, original_requester))
                                elif loop_mode == 'song':
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                            ...
                            async with dj_pool.acquire() as dj_conn:
                                async with dj_conn.cursor(aiomysql.DictCursor) as dj_cur:
                                    ...
                                    if dj_res and dj_res.get('auto_dj'):
                                        ...
                                        await dj_cur.execute("INSERT INTO rhythm_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, 'rhythm', %s, %s, %s)", (guild.id, f"ytsearch:{genre} track", f"📻 Auto-DJ: {genre}", bot.user.id))
                ...
                try:
                    ...
                    await insert_queue_front(cur, "rhythm_queue", guild.id, "rhythm", url, title, requester_id)
                ...
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    ...
                    for orphan in orphans:
                        ...
                        if guild:
                            ...
                            await insert_queue_front(cur, "rhythm_queue", guild.id, "rhythm", orphan['video_url'], orphan.get('title', 'Resumed Track'), bot.user.id)
            ...
            async with conn.cursor() as cur:
                for guild_id, data in list(playback_tracking.items()):
                    ...
                    if guild and guild.voice_client and getattr(guild.voice_client, 'playing', False):
                        ...
                        await cur.execute("REPLACE INTO rhythm_playback_state (guild_id, bot_name, channel_id, video_url, position_seconds, is_playing, title) VALUES (%s, 'rhythm', %s, %s, %s, TRUE, %s)", (guild_id, data['channel_id'], data['url'], pos, data['title']))
                        last_position_persist[guild_id] = now

# --- SETTINGS COMMANDS ---
@bot.tree.command(name="rhythm_main_sethome", description="Set bot's default voice/stage channel")
...
@bot.tree.command(name="rhythm_main_setfeedback", description="Set the text channel for bot announcements")
@commands.has_permissions(administrator=True)
...
@bot.tree.command(name="rhythm_main_djrole", description="Set DJ Role (Admins)")
@commands.has_permissions(administrator=True)
...
@bot.tree.command(name="rhythm_main_removedj", description="Remove DJ Role")
@commands.has_permissions(administrator=True)
...
@bot.tree.command(name="rhythm_main_djmode", description="Toggle Strict DJ Mode")
@commands.has_permissions(administrator=True)
...
@bot.tree.command(name="rhythm_main_247", description="Toggle 24/7 Mode")
@commands.has_permissions(administrator=True)
...
@bot.tree.command(name="rhythm_main_restart", description="Forcefully restart the bot instance (Admins only)")
@commands.has_permissions(administrator=True)
...
# --- PLAYBACK COMMANDS ---
@bot.tree.command(name="rhythm_main_play", description="Play a song, link, livestream, or YouTube playlist")
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                for track in entries_to_add:
                    await cur.execute("INSERT INTO rhythm_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, 'rhythm', %s, %s, %s)", (interaction.guild.id, track.uri, track.title, interaction.user.id))
                    added_count += 1
...
@bot.tree.command(name="rhythm_main_playnext", description="Put song at top of queue")
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute("INSERT INTO rhythm_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, 'rhythm', %s, %s, %s)", (interaction.guild.id, track.uri, track.title, interaction.user.id))
                for r in q: await cur.execute("INSERT INTO rhythm_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, 'rhythm', %s, %s, %s)", (interaction.guild.id, r[1], r[2], r[3]))
    vc = interaction.guild.voice_client
    if not vc or (not getattr(vc, 'playing', False) and not getattr(vc, 'paused', False)):
...
@bot.tree.command(name="rhythm_main_skip", description="Skip current song")
...
@bot.tree.command(name="rhythm_main_stop", description="Stop music and clear state")
...
@bot.tree.command(name="rhythm_main_pause", description="Pause playback")
...
@bot.tree.command(name="rhythm_main_resume", description="Resume playback")
...
@bot.tree.command(name="rhythm_main_clear", description="Clear the queue")
...
@bot.tree.command(name="rhythm_main_join", description="Force bot to join your channel")
...
@bot.tree.command(name="rhythm_main_leave", description="Force bot to leave")
...
@bot.tree.command(name="rhythm_main_queue", description="View queue")
async def queue_cmd(interaction: discord.Interaction):
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT title FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT 10", (interaction.guild.id,))
...
@bot.tree.command(name="rhythm_main_shuffle", description="Shuffle the queue")
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute("DELETE FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm'", (interaction.guild.id,))
                for row in l: await cur.execute("INSERT INTO rhythm_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, 'rhythm', %s, %s, %s)", (interaction.guild.id, row[1], row[2], row[3]))
...
@bot.tree.command(name="rhythm_main_remove", description="Remove song by queue number")
...
@bot.tree.command(name="rhythm_main_skipto", description="Skip to a queue number")
...
@bot.tree.command(name="rhythm_main_move", description="Move song to new position")
...
# --- PLAYLISTS & HISTORY ---
@bot.tree.command(name="rhythm_main_savequeue", description="Save the current queue as a custom personal playlist")
...
@bot.tree.command(name="rhythm_main_loadqueue", description="Load a custom personal playlist into the current queue")
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                for url, title in q:
                    await cur.execute("INSERT INTO rhythm_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, 'rhythm', %s, %s, %s)", (interaction.guild.id, url, title, interaction.user.id))
...
@bot.tree.command(name="rhythm_main_leaderboard", description="Show the top 10 most played tracks in this server")
...
@bot.tree.command(name="rhythm_main_history", description="Show last 5 songs played in the server")
...
@bot.tree.command(name="rhythm_main_userhistory", description="See the last 10 tracks requested by a specific user")
...
@bot.tree.command(name="rhythm_main_steal", description="Steal a song from a user's history and add it to the queue")
...
@bot.tree.command(name="rhythm_main_grab", description="DM yourself the current song")
...
# --- MODIFIERS & FILTERS ---
@bot.tree.command(name="rhythm_main_volume", description="Set volume (1-200)")
...
@bot.tree.command(name="rhythm_main_loop", description="Toggle loop: off, song, queue")
...
@bot.tree.command(name="rhythm_main_filter", description="Apply an audio filter to the music")
...
@bot.tree.command(name="rhythm_main_fade", description="Toggle 5-second smooth fade in/out transitions")
...
@bot.tree.command(name="rhythm_main_modify", description="Change speed and pitch for upcoming tracks")
...
# --- SCRUBBING ---
@bot.tree.command(name="rhythm_main_seek", description="Seek to seconds")
...
@bot.tree.command(name="rhythm_main_forward", description="Skip forward X seconds")
...
@bot.tree.command(name="rhythm_main_rewind", description="Rewind X seconds")
...
@bot.tree.command(name="rhythm_main_replay", description="Restart current song")
...
# --- UTILITY & INFO ---
@bot.tree.command(name="rhythm_main_panel", description="Spawn the advanced music control panel")
            ...
            async with DBPoolManager() as pool:
                async with pool.acquire() as conn:
                    async with conn.cursor() as cur:
                        ...
                        for row in l: await cur.execute("INSERT INTO rhythm_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, 'rhythm', %s, %s, %s)", (i.guild.id, row[1], row[2], row[3]))
...
@bot.tree.command(name="rhythm_main_nowplaying", description="Show song status")
...
@bot.tree.command(name="rhythm_main_ping", description="Bot latency")
...
@bot.tree.command(name="rhythm_main_uptime", description="Bot uptime")
...
@bot.tree.command(name="rhythm_main_stats", description="Bot statistics")
...
@bot.tree.command(name="rhythm_main_help", description="List all RHYTHM commands")
                    ...
                    async with pool.acquire() as conn:
                        async with conn.cursor() as cur:
                            for entry in new_tracks:
                                ...
                                if t_url:
                                    await cur.execute("INSERT INTO rhythm_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, 'rhythm', %s, %s, %s)", (guild_id, t_url, t_title, req_id))
                                    added_count += 1
            ...
            if guild and guild.voice_client:
                ...
                if cmd == 'STOP':
                    async with DBPoolManager() as pool:
                        async with pool.acquire() as conn:
                            async with conn.cursor() as cur:
                                await cur.execute("DELETE FROM rhythm_queue WHERE guild_id = %s AND bot_name = %s", (guild_id, 'rhythm'))
                                ...
                                async with pool.acquire() as conn:
                                    async with conn.cursor() as cur:
                                        for track in entries:
                                            await cur.execute("INSERT INTO rhythm_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, %s, %s, %s, %s)", (guild.id, 'rhythm', track.uri, track.title, bot.user.id))
                                            added_count += 1
                            if playlist_url:
                        ...
                        if _has_human_listeners(guild.voice_client) and not force_leave:
                            ...
                            async with DBPoolManager() as pool:
                                async with pool.acquire() as conn:
                                    async with conn.cursor() as cur:
                                        await cur.execute("DELETE FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm'", (guild.id,))
                                        await cur.execute("REPLACE INTO rhythm_playback_state (guild_id, bot_name, is_playing) VALUES (%s, 'rhythm', FALSE)", (guild.id,))
