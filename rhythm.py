...
WEBHOOK_URL = os.getenv('RHYTHM_WEBHOOK_URL', '').strip()
...
DB_CONFIG = {
    ...
    'db': os.getenv(f"{BOT_ENV_PREFIX}_DB_NAME") or "discord_music_rhythm",
    'autocommit': True,
...
_rhythm_prefetch_cache: dict = {}  # guild_id -> {url, title, track, expires_at}
...
async def lavalink_health_monitor():
    try:
        if not _has_connected_lavalink_node():
            logger.warning("[rhythm] Lavalink health check failed; reconnect task armed.")
            ensure_lavalink_connection_task()
        ...
        logger.exception("[rhythm] Lavalink health monitor failed.")
        ...
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    await cur.execute("SELECT auto_dj FROM rhythm_swarm_toggles WHERE guild_id = %s", (guild_id,))
...
async def set_autodj_enabled(guild_id, enabled):
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor(aiomysql.DictCursor) as cur:
                await cur.execute("INSERT INTO rhythm_swarm_toggles (guild_id, auto_dj) VALUES (%s, %s) ON DUPLICATE KEY UPDATE auto_dj = VALUES(auto_dj)", (guild_id, bool(enabled)))
...
async def get_saved_settings_summary(guild_id):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT home_vc_id, volume, loop_mode, filter_mode, dj_role_id, feedback_channel_id, transition_mode, custom_speed, custom_pitch, custom_modifiers_left, dj_only_mode, stay_in_vc FROM rhythm_guild_settings WHERE guild_id = %s", (guild_id,))
                row = await cur.fetchone()
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute("CREATE TABLE IF NOT EXISTS rhythm_playback_state (guild_id BIGINT, bot_name VARCHAR(50), channel_id BIGINT, video_url TEXT, position_seconds INT DEFAULT 0, is_playing BOOLEAN DEFAULT FALSE, is_paused BOOLEAN DEFAULT FALSE, title TEXT, last_checkpoint_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, play_session_key VARCHAR(64) DEFAULT NULL, track_uid CHAR(32) DEFAULT NULL, PRIMARY KEY (guild_id, bot_name))")
                await safe_schema_execute(cur, "ALTER TABLE rhythm_playback_state ADD COLUMN title TEXT")
                await safe_schema_execute(cur, "ALTER TABLE rhythm_playback_state ADD COLUMN is_paused BOOLEAN DEFAULT FALSE")
                await safe_schema_execute(cur, "ALTER TABLE rhythm_playback_state ADD COLUMN last_checkpoint_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP")
                await safe_schema_execute(cur, "ALTER TABLE rhythm_playback_state ADD COLUMN play_session_key VARCHAR(64) DEFAULT NULL")
                await safe_schema_execute(cur, "ALTER TABLE rhythm_playback_state ADD COLUMN track_uid CHAR(32) DEFAULT NULL")
                await cur.execute("CREATE TABLE IF NOT EXISTS rhythm_guild_settings (guild_id BIGINT PRIMARY KEY, home_vc_id BIGINT, volume INT DEFAULT 100, loop_mode VARCHAR(10) DEFAULT 'queue', filter_mode VARCHAR(20) DEFAULT 'none', dj_role_id BIGINT DEFAULT NULL, feedback_channel_id BIGINT DEFAULT NULL, transition_mode VARCHAR(10) DEFAULT 'off', fade_seconds FLOAT DEFAULT 3.0, fade_curve VARCHAR(20) DEFAULT 'linear', custom_speed FLOAT DEFAULT 1.0, custom_pitch FLOAT DEFAULT 1.0, custom_modifiers_left INT DEFAULT 0, dj_only_mode BOOLEAN DEFAULT FALSE, stay_in_vc BOOLEAN DEFAULT FALSE)")
                await safe_schema_execute(cur, "ALTER TABLE rhythm_guild_settings MODIFY loop_mode VARCHAR(10) DEFAULT 'queue'")
                await safe_schema_execute(cur, "ALTER TABLE rhythm_guild_settings ADD COLUMN fade_seconds FLOAT DEFAULT 3.0")
                await safe_schema_execute(cur, "ALTER TABLE rhythm_guild_settings ADD COLUMN fade_curve VARCHAR(20) DEFAULT 'linear'")
                await safe_schema_execute(cur, "ALTER TABLE rhythm_guild_settings ADD COLUMN filter_stack VARCHAR(20) DEFAULT NULL")
                await safe_schema_execute(cur, "UPDATE rhythm_guild_settings SET loop_mode = 'queue' WHERE loop_mode IS NULL OR loop_mode NOT IN ('off', 'song', 'queue')")
                await cur.execute("CREATE TABLE IF NOT EXISTS rhythm_queue (id INT AUTO_INCREMENT PRIMARY KEY, guild_id BIGINT, bot_name VARCHAR(50), video_url TEXT, title TEXT, requester_id BIGINT DEFAULT NULL, track_uid CHAR(32) DEFAULT NULL)")
                await cur.execute("CREATE TABLE IF NOT EXISTS rhythm_queue_backup (id INT AUTO_INCREMENT PRIMARY KEY, guild_id BIGINT, bot_name VARCHAR(50), video_url TEXT, title TEXT, requester_id BIGINT DEFAULT NULL, track_uid CHAR(32) DEFAULT NULL)")
                await safe_schema_execute(cur, "ALTER TABLE rhythm_queue ADD COLUMN bot_name VARCHAR(50)")
                await safe_schema_execute(cur, "ALTER TABLE rhythm_queue_backup ADD COLUMN bot_name VARCHAR(50)")
                await safe_schema_execute(cur, "ALTER TABLE rhythm_queue ADD COLUMN requester_id BIGINT DEFAULT NULL")
                await safe_schema_execute(cur, "ALTER TABLE rhythm_queue_backup ADD COLUMN requester_id BIGINT DEFAULT NULL")
                await safe_schema_execute(cur, "ALTER TABLE rhythm_queue ADD COLUMN track_uid CHAR(32) DEFAULT NULL")
                await safe_schema_execute(cur, "ALTER TABLE rhythm_queue_backup ADD COLUMN track_uid CHAR(32) DEFAULT NULL")
                await cur.execute("CREATE TABLE IF NOT EXISTS rhythm_history (id INT AUTO_INCREMENT PRIMARY KEY, guild_id BIGINT, video_url TEXT, title TEXT, played_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, requester_id BIGINT DEFAULT NULL)")
                await safe_schema_execute(cur, "ALTER TABLE rhythm_history ADD COLUMN requester_id BIGINT DEFAULT NULL")
                await cur.execute("CREATE TABLE IF NOT EXISTS rhythm_user_playlists (id INT AUTO_INCREMENT PRIMARY KEY, user_id BIGINT, playlist_name VARCHAR(255), video_url TEXT, title TEXT)")
                await cur.execute("CREATE TABLE IF NOT EXISTS rhythm_track_intelligence (guild_id BIGINT NOT NULL, url_key VARCHAR(64) NOT NULL, video_url TEXT, title TEXT, queued_count INT NOT NULL DEFAULT 0, play_count INT NOT NULL DEFAULT 0, finish_count INT NOT NULL DEFAULT 0, skip_count INT NOT NULL DEFAULT 0, like_count INT NOT NULL DEFAULT 0, dislike_count INT NOT NULL DEFAULT 0, total_listen_seconds INT NOT NULL DEFAULT 0, last_requester_id BIGINT DEFAULT NULL, source VARCHAR(40) DEFAULT 'unknown', first_seen TIMESTAMP DEFAULT CURRENT_TIMESTAMP, last_queued TIMESTAMP NULL DEFAULT NULL, last_played TIMESTAMP NULL DEFAULT NULL, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, PRIMARY KEY (guild_id, url_key))")
                await cur.execute("CREATE TABLE IF NOT EXISTS rhythm_user_track_affinity (guild_id BIGINT NOT NULL, user_id BIGINT NOT NULL, url_key VARCHAR(64) NOT NULL, video_url TEXT, title TEXT, queued_count INT NOT NULL DEFAULT 0, play_count INT NOT NULL DEFAULT 0, finish_count INT NOT NULL DEFAULT 0, skip_count INT NOT NULL DEFAULT 0, like_count INT NOT NULL DEFAULT 0, dislike_count INT NOT NULL DEFAULT 0, score FLOAT NOT NULL DEFAULT 0, last_requested TIMESTAMP NULL DEFAULT NULL, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, PRIMARY KEY (guild_id, user_id, url_key))")
                await cur.execute("CREATE TABLE IF NOT EXISTS rhythm_smart_recommendations (id INT AUTO_INCREMENT PRIMARY KEY, guild_id BIGINT NOT NULL, requester_id BIGINT DEFAULT NULL, seed_title TEXT, seed_url TEXT, query_text TEXT, chosen_url TEXT, chosen_title TEXT, reason VARCHAR(80), accepted BOOLEAN DEFAULT TRUE, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)")
                await cur.execute("CREATE TABLE IF NOT EXISTS rhythm_track_embeddings (url_key VARCHAR(64) NOT NULL PRIMARY KEY, title TEXT, video_url TEXT, embedding MEDIUMTEXT, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP)")
                await cur.execute("CREATE TABLE IF NOT EXISTS rhythm_track_cooccurrence (guild_id BIGINT NOT NULL, url_key_a VARCHAR(64) NOT NULL, url_key_b VARCHAR(64) NOT NULL, weight FLOAT NOT NULL DEFAULT 0, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, PRIMARY KEY (guild_id, url_key_a, url_key_b))")
                await safe_schema_execute(cur, "CREATE INDEX rhythm_track_intelligence_recent_idx ON rhythm_track_intelligence (guild_id, last_played)")
                await safe_schema_execute(cur, "CREATE INDEX rhythm_track_intelligence_requester_idx ON rhythm_track_intelligence (guild_id, last_requester_id, last_played)")
                await safe_schema_execute(cur, "CREATE INDEX rhythm_user_affinity_recent_idx ON rhythm_user_track_affinity (guild_id, user_id, last_requested)")
                await safe_schema_execute(cur, "CREATE INDEX rhythm_smart_recommendations_recent_idx ON rhythm_smart_recommendations (guild_id, created_at)")
                await safe_schema_execute(cur, "CREATE INDEX rhythm_track_intelligence_global_idx ON rhythm_track_intelligence (url_key, last_played)")
                await safe_schema_execute(cur, "CREATE INDEX rhythm_track_cooccurrence_weight_idx ON rhythm_track_cooccurrence (guild_id, url_key_a, weight)")
                await cur.execute("CREATE TABLE IF NOT EXISTS rhythm_bot_home_channels (guild_id BIGINT, bot_name VARCHAR(50), home_vc_id BIGINT, PRIMARY KEY (guild_id, bot_name))")
                await cur.execute("CREATE TABLE IF NOT EXISTS rhythm_voice_state (guild_id BIGINT, bot_name VARCHAR(50), last_channel_id BIGINT NULL, connected_channel_id BIGINT NULL, text_channel_id BIGINT NULL, disconnected_at TIMESTAMP NULL, last_seen_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, desired_connected BOOLEAN DEFAULT FALSE, reconnect_attempts INT NOT NULL DEFAULT 0, last_error TEXT NULL, PRIMARY KEY (guild_id, bot_name))")
                await cur.execute("CREATE TABLE IF NOT EXISTS rhythm_metrics (guild_id BIGINT, bot_name VARCHAR(50), voice_connected BOOLEAN DEFAULT FALSE, connected_channel_id BIGINT NULL, player_connected BOOLEAN DEFAULT FALSE, player_playing BOOLEAN DEFAULT FALSE, player_paused BOOLEAN DEFAULT FALSE, queue_count INT DEFAULT 0, backup_queue_count INT DEFAULT 0, is_playing_db BOOLEAN DEFAULT FALSE, is_paused_db BOOLEAN DEFAULT FALSE, position_seconds INT DEFAULT 0, recovery_pending BOOLEAN DEFAULT FALSE, heartbeat_age_seconds INT DEFAULT 0, lavalink_ready BOOLEAN DEFAULT FALSE, last_error TEXT NULL, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, PRIMARY KEY (guild_id, bot_name))")
                await safe_schema_execute(cur, "ALTER TABLE rhythm_voice_state ADD COLUMN reconnect_attempts INT NOT NULL DEFAULT 0")
                await safe_schema_execute(cur, "ALTER TABLE rhythm_voice_state ADD COLUMN last_error TEXT NULL")
                await safe_schema_execute(cur, "CREATE INDEX rhythm_voice_state_rejoin_idx ON rhythm_voice_state (bot_name, desired_connected, last_seen_at)")
                await safe_schema_execute(cur, "CREATE INDEX rhythm_metrics_status_idx ON rhythm_metrics (bot_name, updated_at)")
                await safe_schema_execute(cur, "ALTER TABLE rhythm_metrics ADD COLUMN duration_seconds INT DEFAULT 0")
                await cur.execute("CREATE TABLE IF NOT EXISTS rhythm_active_playlists (guild_id BIGINT, bot_name VARCHAR(50), playlist_url TEXT, known_track_count INT DEFAULT 0, requester_id BIGINT, channel_id BIGINT DEFAULT NULL, PRIMARY KEY (guild_id, bot_name))")
                await cur.execute("CREATE TABLE IF NOT EXISTS rhythm_active_playlist_tracks (guild_id BIGINT, bot_name VARCHAR(50), playlist_url TEXT, position_idx INT DEFAULT 0, track_key CHAR(40), track_uid CHAR(32) DEFAULT NULL, video_url TEXT, title TEXT, requester_id BIGINT DEFAULT NULL, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP)")
                await safe_schema_execute(cur, "ALTER TABLE rhythm_active_playlists ADD COLUMN channel_id BIGINT DEFAULT NULL")
                await safe_schema_execute(cur, "ALTER TABLE rhythm_active_playlist_tracks ADD COLUMN track_uid CHAR(32) DEFAULT NULL")
                await cur.execute("CREATE TABLE IF NOT EXISTS rhythm_swarm_toggles (guild_id BIGINT PRIMARY KEY, auto_dj BOOLEAN DEFAULT FALSE, audio_filter VARCHAR(20) DEFAULT 'normal')")
                await cur.execute("CREATE TABLE IF NOT EXISTS rhythm_swarm_overrides (guild_id BIGINT, bot_name VARCHAR(50), command VARCHAR(20), PRIMARY KEY(guild_id, bot_name))")
                await cur.execute("CREATE TABLE IF NOT EXISTS rhythm_swarm_direct_orders (id INT AUTO_INCREMENT PRIMARY KEY, bot_name VARCHAR(50), guild_id BIGINT, vc_id BIGINT, text_channel_id BIGINT, command VARCHAR(50), data TEXT, attempts INT NOT NULL DEFAULT 0, last_error TEXT NULL, claimed_at TIMESTAMP NULL, claim_token VARCHAR(128) NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)")
                ...
                await safe_schema_execute(cur, "CREATE INDEX rhythm_queue_lookup_idx ON rhythm_queue (guild_id, bot_name, id)")
                await safe_schema_execute(cur, "CREATE INDEX rhythm_queue_backup_lookup_idx ON rhythm_queue_backup (guild_id, bot_name, id)")
                await safe_schema_execute(cur, "CREATE INDEX rhythm_queue_track_uid_idx ON rhythm_queue (guild_id, bot_name, track_uid)")
                await safe_schema_execute(cur, "CREATE INDEX rhythm_queue_backup_track_uid_idx ON rhythm_queue_backup (guild_id, bot_name, track_uid)")
                await safe_schema_execute(cur, "CREATE INDEX rhythm_history_guild_played_idx ON rhythm_history (guild_id, played_at)")
                await safe_schema_execute(cur, "CREATE INDEX rhythm_history_requester_idx ON rhythm_history (guild_id, requester_id, played_at)")
                await safe_schema_execute(cur, "CREATE INDEX rhythm_playback_resume_idx ON rhythm_playback_state (bot_name, is_playing, guild_id)")
                await safe_schema_execute(cur, "CREATE INDEX rhythm_playback_track_uid_idx ON rhythm_playback_state (bot_name, track_uid)")
                await safe_schema_execute(cur, "CREATE INDEX rhythm_playlist_bot_idx ON rhythm_active_playlists (bot_name, guild_id)")
                ...
                await safe_schema_execute(cur, "CREATE INDEX rhythm_playlist_track_uid_idx ON rhythm_active_playlist_tracks (guild_id, bot_name, track_uid)")
                await safe_schema_execute(cur, "CREATE INDEX rhythm_active_playlist_tracks_guild_bot_idx ON rhythm_active_playlist_tracks (guild_id, bot_name, position_idx)")
                await safe_schema_execute(cur, "CREATE INDEX rhythm_active_playlist_tracks_del_idx ON rhythm_active_playlist_tracks (guild_id, bot_name, playlist_url(255))")
                await safe_schema_execute(cur, "CREATE INDEX rhythm_user_playlist_lookup_idx ON rhythm_user_playlists (user_id, playlist_name)")
                await safe_schema_execute(cur, "DROP INDEX rhythm_queue_track_uid_idx ON rhythm_queue")
                await safe_schema_execute(cur, "DROP INDEX rhythm_queue_backup_track_uid_idx ON rhythm_queue_backup")
                await safe_schema_execute(cur, "ALTER TABLE rhythm_playback_state ADD COLUMN bot_name VARCHAR(50) DEFAULT 'rhythm'")
                await safe_schema_execute(cur, "ALTER TABLE rhythm_active_playlists ADD COLUMN bot_name VARCHAR(50) DEFAULT 'rhythm'")
                await safe_schema_execute(cur, "ALTER TABLE rhythm_bot_home_channels ADD COLUMN bot_name VARCHAR(50) DEFAULT 'rhythm'")
                await cur.execute(
                    "UPDATE rhythm_queue SET track_uid = SUBSTRING(SHA2(CONCAT(UUID(), RAND(), id, COALESCE(video_url, ''), COALESCE(title, '')), 256), 1, 32) "
                    "WHERE track_uid IS NULL OR track_uid = ''"
                )
                await cur.execute(
                    "UPDATE rhythm_queue_backup SET track_uid = SUBSTRING(SHA2(CONCAT(UUID(), RAND(), id, COALESCE(video_url, ''), COALESCE(title, '')), 256), 1, 32) "
                    "WHERE track_uid IS NULL OR track_uid = ''"
                )
                await cur.execute(
                    "UPDATE rhythm_active_playlist_tracks SET track_uid = SUBSTRING(SHA2(CONCAT(UUID(), RAND(), guild_id, COALESCE(playlist_url, ''), position_idx, COALESCE(video_url, ''), COALESCE(title, '')), 256), 1, 32) "
                    "WHERE track_uid IS NULL OR track_uid = ''"
                )
                await cur.execute(
                    "UPDATE rhythm_playback_state SET track_uid = SUBSTRING(SHA2(CONCAT(UUID(), RAND(), guild_id, COALESCE(video_url, ''), COALESCE(title, '')), 256), 1, 32) "
                    "WHERE (video_url IS NOT NULL OR title IS NOT NULL) AND (track_uid IS NULL OR track_uid = '')"
                )
                await cur.execute(
                    "UPDATE rhythm_playback_state SET play_session_key = track_uid "
                    "WHERE track_uid IS NOT NULL AND track_uid <> ''"
                )
                await dedupe_queue_table_by_track_uid(cur, "rhythm_queue", bot_name="rhythm")
                await dedupe_queue_table_by_track_uid(cur, "rhythm_queue_backup", bot_name="rhythm")
                ...
                _trim_live = await trim_queue_duplicate_runs(cur, "rhythm_queue", bot_name="rhythm")
                _trim_backup = await trim_queue_duplicate_runs(cur, "rhythm_queue_backup", bot_name="rhythm")
                ...
                await cur.execute(
                    ...
                    "WHERE table_schema = DATABASE() AND table_name = 'rhythm_queue' "
                    "AND index_name = 'rhythm_queue_track_uid_unique_idx'"
                )
                _uid_unique_idx_row = await cur.fetchone()
                if not (_uid_unique_idx_row and _uid_unique_idx_row[0]):
                    removed_legacy_live = await dedupe_queue_table_by_track_key(cur, "rhythm_queue", bot_name="rhythm")
                    removed_legacy_backup = await dedupe_queue_table_by_track_key(cur, "rhythm_queue_backup", bot_name="rhythm")
                ...
                await safe_schema_execute(cur, "CREATE UNIQUE INDEX rhythm_queue_track_uid_unique_idx ON rhythm_queue (guild_id, bot_name, track_uid)")
                await safe_schema_execute(cur, "CREATE UNIQUE INDEX rhythm_queue_backup_track_uid_unique_idx ON rhythm_queue_backup (guild_id, bot_name, track_uid)")
                await safe_schema_execute(cur, "CREATE INDEX rhythm_active_playlists_lookup_idx ON rhythm_active_playlists (guild_id, bot_name)")
                await safe_schema_execute(cur, "CREATE INDEX rhythm_error_events_guild_time_idx ON rhythm_error_events (guild_id, created_at)")
                playlist_db_initialized = True
                logger.info("Database tables verified/created for RHYTHM.")
...
async def ensure_guild_settings(guild_id):
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("INSERT IGNORE INTO rhythm_guild_settings (guild_id) VALUES (%s)", (guild_id,))
...
async def get_or_fetch_track_embedding(cur, url_key, title, video_url=None):
    ...
    await cur.execute("SELECT embedding FROM rhythm_track_embeddings WHERE url_key = %s", (url_key,))
    row = await cur.fetchone()
    if row:
    ...
    if vector:
        try:
            await cur.execute(
                "INSERT INTO rhythm_track_embeddings (url_key, title, video_url, embedding) VALUES (%s, %s, %s, %s) "
...
async def find_similar_tracks_global(cur, seed_vector, avoid_keys, limit=5):
    ...
    if not seed_vector:
        return []
    await cur.execute("SELECT url_key, title, video_url, embedding FROM rhythm_track_embeddings ORDER BY updated_at DESC LIMIT 1500")
...
async def record_track_cooccurrence(cur, guild_id, prev_url, prev_title, next_url, next_title):
    ...
    if key_a == key_b:
        return
    await cur.execute(
        "INSERT INTO rhythm_track_cooccurrence (guild_id, url_key_a, url_key_b, weight) VALUES (%s, %s, %s, 1) "
...
async def bulk_record_tracks_queued(cur, guild_id, tracks):
    ...
    if intelligence_rows:
        await cur.executemany(
            "INSERT INTO rhythm_track_intelligence (guild_id, url_key, video_url, title, queued_count, last_requester_id, last_queued, source) "
    ...
    if affinity_rows:
        await cur.executemany(
            "INSERT INTO rhythm_user_track_affinity (guild_id, user_id, url_key, video_url, title, queued_count, last_requested, score) "
...
async def record_track_play_started(cur, guild_id, video_url, title, requester_id):
    url_key = _track_key(video_url, title)
    await cur.execute(
        "INSERT INTO rhythm_track_intelligence (guild_id, url_key, video_url, title, play_count, last_requester_id, last_played, source) "
    ...
    if requester_id:
        await cur.execute(
            "INSERT INTO rhythm_user_track_affinity (guild_id, user_id, url_key, video_url, title, play_count, last_requested, score) "
...
async def record_track_play_resumed(cur, guild_id, video_url, title, requester_id):
    ...
    url_key = _track_key(video_url, title)
    await cur.execute(
        "INSERT INTO rhythm_track_intelligence (guild_id, url_key, video_url, title, last_requester_id, last_played, source) "
    ...
    if requester_id:
        await cur.execute(
            "INSERT INTO rhythm_user_track_affinity (guild_id, user_id, url_key, video_url, title, last_requested, score) "
...
async def record_track_outcome(cur, guild_id, video_url, title, requester_id, *, outcome, listen_seconds=0):
    ...
    await cur.execute(
        "INSERT INTO rhythm_track_intelligence (guild_id, url_key, video_url, title, finish_count, skip_count, total_listen_seconds, last_requester_id, last_played, source) "
    ...
    if requester_id:
        await cur.execute(
            "INSERT INTO rhythm_user_track_affinity (guild_id, user_id, url_key, video_url, title, finish_count, skip_count, score, last_requested) "
...
async def record_track_feedback(cur, guild_id, user_id, video_url, title, liked=True):
    ...
    await cur.execute(
        "INSERT INTO rhythm_track_intelligence (guild_id, url_key, video_url, title, like_count, dislike_count, last_requester_id, source) "
    ...
    await cur.execute(
        "INSERT INTO rhythm_user_track_affinity (guild_id, user_id, url_key, video_url, title, like_count, dislike_count, score, last_requested) "
        "VALUES (%s, %s, %s, %s, %s, %s, %s, %s, NOW()) "
...
async def get_current_track_snapshot(guild_id):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute(
                    "SELECT video_url, title, track_uid FROM rhythm_playback_state WHERE guild_id = %s AND bot_name = 'rhythm' AND (is_playing = TRUE OR is_paused = TRUE) ORDER BY is_playing DESC LIMIT 1",
                    (guild_id,),
                )
                row = await cur.fetchone()
    if not row:
...
async def load_smart_avoid_keys(cur, guild_id, listener_ids=None):
    avoid = set()
    await cur.execute("SELECT video_url, title FROM rhythm_history WHERE guild_id = %s ORDER BY played_at DESC LIMIT %s", (guild_id, SMART_RECENT_HISTORY_LIMIT))
    for row in await cur.fetchall():
        avoid.add(_track_key(_row_value(row, "video_url", _row_value(row, 0)), _row_value(row, "title", _row_value(row, 1))))
    await cur.execute("SELECT video_url, title FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT %s", (guild_id, SMART_RECENT_HISTORY_LIMIT))
    for row in await cur.fetchall():
        avoid.add(_track_key(_row_value(row, "video_url", _row_value(row, 0)), _row_value(row, "title", _row_value(row, 1))))
    ...
    if ids:
        ...
        await cur.execute(
            f"SELECT url_key FROM rhythm_user_track_affinity WHERE guild_id = %s AND user_id IN ({placeholders}) AND dislike_count > like_count",
            (guild_id, *ids),
        )
        for row in await cur.fetchall():
...
async def build_smart_recommendation(cur, guild_id, listener_ids=None):
    ...
    if ids:
        ...
        await cur.execute(
            f"""SELECT title, video_url,
                ...
                FROM rhythm_user_track_affinity
    ...
    await cur.execute(
        f"""SELECT title, video_url,
            ...
            FROM rhythm_track_intelligence
    ...
    await cur.execute("SELECT video_url, title FROM rhythm_history WHERE guild_id = %s ORDER BY played_at DESC LIMIT 1", (guild_id,))
    ...
    if last_row:
        ...
        await cur.execute(
            """SELECT ti.title AS title, ti.video_url AS video_url,
                ...
                FROM rhythm_track_cooccurrence tc
                JOIN rhythm_track_intelligence ti ON ti.guild_id = tc.guild_id AND ti.url_key = tc.url_key_b
    ...
    if not candidates:
        await cur.execute("SELECT title, video_url, 1.0 AS weight, 'recent history' AS reason FROM rhythm_history WHERE guild_id = %s ORDER BY played_at DESC LIMIT %s", (guild_id, SMART_SEED_POOL_LIMIT))
        candidates.extend(await cur.fetchall())
...
async def record_smart_recommendation(cur, guild_id, requester_id, recommendation, chosen, *, reason):
    await cur.execute(
        "INSERT INTO rhythm_smart_recommendations (guild_id, requester_id, seed_title, seed_url, query_text, chosen_url, chosen_title, reason) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
        (
            guild_id,
            requester_id,
...
async def persist_voice_state(guild_id, channel_id=None, *, text_channel_id=None, desired_connected=True, connected=True, last_error=None):
    ...
    try:
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    await cur.execute(
                        """
                        INSERT INTO rhythm_voice_state
        ...
        logger.exception("[rhythm] Failed to persist voice state for guild %s", guild_id)
        ...
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    if position is None:
                        await cur.execute(
                            "UPDATE rhythm_playback_state SET is_playing = FALSE, is_paused = FALSE WHERE guild_id = %s AND bot_name = 'rhythm'",
                            (guild_id,),
                        )
                    else:
                        await cur.execute(
                            "UPDATE rhythm_playback_state SET is_playing = FALSE, is_paused = FALSE, position_seconds = %s WHERE guild_id = %s AND bot_name = 'rhythm'",
                            (normalize_position_seconds(position), guild_id),
                        )
    except Exception as tx_error:
        logger.exception("[rhythm] Failed to mark playback disconnected for guild %s", guild_id)
            ...
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    if not actual_connected:
                        ...
                        await cur.execute(
                            "UPDATE rhythm_playback_state SET channel_id = COALESCE(%s, channel_id), is_playing = %s, is_paused = %s, position_seconds = %s WHERE guild_id = %s AND bot_name = 'rhythm'",
                            (actual_channel_id, actual_playing, actual_paused, live_position, guild.id),
                        )
    except Exception as tx_error:
        logger.exception("[rhythm] Failed to reconcile playback state for guild %s", guild.id)
...
async def collect_and_persist_metrics(guild=None):
    ...
    async def _fetch_guild_queue_counts(pool, g):
        ...
        Note: does NOT join rhythm_playback_state — reconcile_runtime_playback_state()
            ...
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    await cur.execute(
                        """
                        SELECT
                            (SELECT COUNT(*) FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm') AS queue_total,
                            (SELECT COUNT(*) FROM rhythm_queue_backup WHERE guild_id = %s AND bot_name = 'rhythm') AS backup_total
                        """,
                        (g.id, g.id),
                    )
                    return await cur.fetchone() or {}
        except Exception:
            logger.debug("[rhythm] Per-guild queue-count SELECT failed for guild %s", g.id, exc_info=True)
            return {}

    try:
        async with DBPoolManager() as pool:
            # Phase 1: fan-out all per-guild queue-count SELECTs in parallel (each on its own connection).
            # rhythm_playback_state is intentionally NOT re-read here — reconcile_runtime_playback_state()
                ...
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    ...
                    if metrics_rows:
                        await cur.executemany(
                            ...
                            VALUES (%s, 'rhythm', %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, 0, %s, %s)
                            """,
                            metrics_rows,
                        )
    except Exception as exc:
        logger.exception("[rhythm] Metrics collection failed: %s", exc)
    ...
    try:
        ...
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    await cur.execute(
                        "SELECT guild_id, COALESCE(connected_channel_id, last_channel_id) AS channel_id, reconnect_attempts FROM rhythm_voice_state WHERE bot_name = 'rhythm' AND desired_connected = TRUE AND COALESCE(connected_channel_id, last_channel_id) IS NOT NULL"
                    )
                    ...
                    async with DBPoolManager() as _pool:
                        async with _pool.acquire() as _conn:
                            async with _conn.cursor() as _cur:
                                await _cur.execute("SELECT stay_in_vc FROM rhythm_guild_settings WHERE guild_id = %s", (guild.id,))
                                stay_row = await _cur.fetchone()
                                stay_in_vc = bool(stay_row[0]) if stay_row else False
                except Exception as tx_error:
                    logger.debug("[rhythm] Failed checking 24/7 state for persistent voice restore.", exc_info=True)
                ...
                if vc:
                    ...
                    try:
                        async with DBPoolManager() as _pool:
                            async with _pool.acquire() as _conn:
                                async with _conn.cursor() as _cur:
                                    await _cur.execute("UPDATE rhythm_voice_state SET reconnect_attempts = reconnect_attempts + 1, last_error = %s WHERE guild_id = %s AND bot_name = 'rhythm'", ("rejoin_failed", guild.id))
                    except Exception as tx_error:
                        logger.debug("[rhythm] Failed to increment reconnect_attempts for guild %s.", guild.id, exc_info=True)
            except Exception as row_exc:
                logger.warning("[rhythm] Persistent voice row recovery failed: %s", row_exc)
    except Exception as exc:
        logger.exception("[rhythm] Persistent voice restore failed: %s", exc)
    ...
    try:
        ...
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    await cur.execute(
                        "UPDATE rhythm_playback_state SET is_paused = %s, is_playing = %s, position_seconds = %s WHERE guild_id = %s AND bot_name = 'rhythm'",
        ...
        logger.exception("[rhythm] Failed to sync pause state for guild %s.", guild_id)
...
async def snapshot_queue_backup(cur, guild_id):
    removed_live_dupes = await dedupe_queue_table_by_track_uid(cur, "rhythm_queue", guild_id=guild_id, bot_name="rhythm")
    ...
    await cur.execute("SELECT video_url, title, requester_id, track_uid FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC", (guild_id,))
    rows = list(await cur.fetchall() or [])
    insert_data = []
    seen_track_uids = set()
    ...
    try:
        ...
        await cur.execute("DELETE FROM rhythm_queue_backup WHERE guild_id = %s AND bot_name = 'rhythm'", (guild_id,))
        if insert_data:
            await cur.executemany(
                """
                INSERT INTO rhythm_queue_backup (guild_id, bot_name, video_url, title, requester_id, track_uid)
                VALUES (%s, %s, %s, %s, %s, %s)
                ON DUPLICATE KEY UPDATE
                    video_url = VALUES(video_url),
                    title = VALUES(title),
...
async def backup_track(cur, guild_id, video_url, title, requester_id, track_uid=None):
    track_uid = _ensure_track_uid(track_uid)
    await cur.execute(
        "SELECT COUNT(*) FROM rhythm_queue_backup WHERE guild_id = %s AND bot_name = 'rhythm' AND track_uid = %s",
        (guild_id, track_uid),
    )
    ...
    try:
        await cur.execute(
            "INSERT INTO rhythm_queue_backup (guild_id, bot_name, video_url, title, requester_id, track_uid) VALUES (%s, 'rhythm', %s, %s, %s, %s)",
...
async def delete_backup_track(cur, guild_id, *, track_uid=None, video_url=None, title=None):
    ...
    if normalized_uid:
        await cur.execute(
            "DELETE FROM rhythm_queue_backup WHERE guild_id = %s AND bot_name = 'rhythm' AND track_uid = %s LIMIT 1",
    ...
    if not video_url and not title:
        return 0
    await cur.execute(
        "DELETE FROM rhythm_queue_backup WHERE guild_id = %s AND bot_name = 'rhythm' AND video_url = %s AND title = %s LIMIT 1",
...
async def enqueue_track(cur, guild_id, video_url, title, requester_id, *, backup=True, track_uid=None):
    ...
    for attempt in range(5):
        try:
            await cur.execute(
                "INSERT INTO rhythm_queue (guild_id, bot_name, video_url, title, requester_id, track_uid) VALUES (%s, 'rhythm', %s, %s, %s, %s)",
                (guild_id, video_url, title, requester_id, track_uid),
            )
            break
        except aiomysql.IntegrityError as e:
    ...
    if backup:
        try:
            ...
            logger.debug("[rhythm] Track intelligence queue write skipped.", exc_info=True)
...
async def delete_live_queue_copies(cur, guild_id, video_url, title, track_uid=None):
    ...
    if normalized_uid:
        await cur.execute(
            "DELETE FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' AND track_uid = %s",
            (guild_id, normalized_uid),
        )
        return max(0, int(getattr(cur, "rowcount", 0) or 0))
    await cur.execute(
        "SELECT id, video_url, title, track_uid FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm'",
        (guild_id,),
    )
    rows = list(await cur.fetchall() or [])
    deleted = 0
    for row in rows:
        ...
        if row_id is None:
            continue
        await cur.execute(
            "DELETE FROM rhythm_queue WHERE id = %s AND guild_id = %s AND bot_name = 'rhythm'",
            (row_id, guild_id),
        )
        deleted += max(0, int(getattr(cur, "rowcount", 0) or 0))
    return deleted
...
async def _fetch_shuffle_weights(cur, guild_id, rows):
    ...
    try:
        ...
        await cur.execute(
            ...
            f"FROM rhythm_track_intelligence WHERE guild_id = %s AND url_key IN ({placeholders})",
            (guild_id, *url_keys),
        )
...
async def shuffle_queue_rows(cur, guild_id, *, preserve_first=True):
    await cur.execute("SELECT id, guild_id, bot_name, video_url, title, requester_id, track_uid FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC", (guild_id,))
    rows = list(await cur.fetchall() or [])
    if len(rows) <= 1:
        return len(rows)
    head = []
    ...
    try:
        await cur.execute("START TRANSACTION")
        await cur.execute("DELETE FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm'", (guild_id,))
        insert_data = [
            (
        ...
        if insert_data:
            await cur.executemany(
                "INSERT INTO rhythm_queue (guild_id, bot_name, video_url, title, requester_id, track_uid) VALUES (%s, %s, %s, %s, %s, %s)",
                insert_data,
            )
        await cur.execute("COMMIT")
    except Exception as tx_error:
        try:
            ...
            logger.debug("Suppressed exception", exc_info=True)
            pass
        logger.exception("[rhythm] Queue reshuffle transaction failed; live queue was rolled back instead of leaking tracks.")
        raise
    return len(rows)
...
async def requeue_finished_track(cur, guild_id, video_url, title, requester_id, track_uid=None):
    ...
    if video_url:
        await cur.execute(
            "SELECT COUNT(*) FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' AND video_url = %s",
            (guild_id, video_url),
        )
    ...
    if original_uid and actual_uid and actual_uid != original_uid:
        try:
            await cur.execute(
                "UPDATE rhythm_queue_backup SET track_uid = %s "
                "WHERE guild_id = %s AND bot_name = 'rhythm' AND track_uid = %s",
...
async def prime_loop_queue_defaults(cur, guild_id, *, shuffle_if_needed=True):
    await cur.execute(
        "INSERT INTO rhythm_guild_settings (guild_id, loop_mode) VALUES (%s, 'queue') ON DUPLICATE KEY UPDATE loop_mode = 'queue'",
        (guild_id,),
    )
    return 0

async def restore_queue_from_backup(cur, guild_id, requester_id=None):
    await cur.execute("SELECT COUNT(*) FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm'", (guild_id,))
    queue_count_row = await cur.fetchone()
    queue_count = queue_count_row[0] if queue_count_row else 0
    if queue_count:
        return 0

    removed_backup_dupes = await dedupe_queue_table_by_track_uid(cur, "rhythm_queue_backup", guild_id=guild_id, bot_name="rhythm")
    ...
    await cur.execute("SELECT video_url, title, requester_id, track_uid FROM rhythm_queue_backup WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC", (guild_id,))
    rows = list(await cur.fetchall() or [])
    if not rows:
        return 0
    ...
    for row in rows:
        ...
        insert_data.append((guild_id, 'rhythm', url, title, requester_id if requester_id is not None else backup_requester_id, track_uid))
    if insert_data:
        await cur.executemany(
            """
            INSERT INTO rhythm_queue (guild_id, bot_name, video_url, title, requester_id, track_uid)
            VALUES (%s, %s, %s, %s, %s, %s)
            ON DUPLICATE KEY UPDATE
                video_url = VALUES(video_url),
                title = VALUES(title),
...
async def repair_queue_backup_parity(cur, guild_id, *, reason="queue_integrity", active_player=None):
    ...
    await cur.execute("SELECT id, video_url, title, requester_id, track_uid FROM rhythm_queue_backup WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC", (guild_id,))
    backup_rows = list(await cur.fetchall() or [])
    await cur.execute("SELECT id, video_url, title, requester_id, track_uid FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC", (guild_id,))
    live_rows = list(await cur.fetchall() or [])
    if not backup_rows and not live_rows:
        return 0, 0

    await cur.execute(
        "SELECT video_url, title, is_playing, is_paused, position_seconds, track_uid FROM rhythm_playback_state WHERE guild_id = %s AND bot_name = 'rhythm' LIMIT 1",
        (guild_id,),
    )
...
async def restore_active_playback_entry(cur, guild_id, requester_id=None):
    await cur.execute(
        "SELECT video_url, title, track_uid FROM rhythm_playback_state "
        "WHERE guild_id = %s AND bot_name = 'rhythm' "
...
async def requeue_failed_track(cur, guild_id, channel_id, url, title, requester_id, *, position=0, reason="recovery", track_uid=None):
    ...
    async with get_track_requeue_lock(guild_id):
        ...
        if not original_track_uid:
            await cur.execute("SELECT track_uid FROM rhythm_playback_state WHERE guild_id = %s AND bot_name = 'rhythm' LIMIT 1", (guild_id,))
            ...
            try:
                ...
                if normalized_track_uid:
                    await cur.execute(
                        "SELECT video_url, title, requester_id, track_uid FROM rhythm_queue_backup WHERE guild_id = %s AND bot_name = 'rhythm' AND track_uid = %s ORDER BY id ASC LIMIT 1",
                        (guild_id, normalized_track_uid),
                    )
                else:
                    await cur.execute(
                        "SELECT video_url, title, requester_id, track_uid FROM rhythm_queue_backup WHERE guild_id = %s AND bot_name = 'rhythm' AND (video_url = %s OR title = %s) ORDER BY id ASC LIMIT 1",
                        (guild_id, url, title),
                    )
                ...
                logger.debug("[rhythm] Backup row lookup skipped while requeueing failed track.", exc_info=True)
        ...
        if recent_until and now < recent_until:
            ...
            await insert_queue_front(cur, "rhythm_queue", guild_id, "rhythm", url, title, requester_id, track_uid)
            if guild_id in playback_tracking: playback_tracking[guild_id]["track_uid"] = track_uid
            await remember_recovery_state(guild_id, channel_id, position)
        ...
        await cur.execute(
            "UPDATE rhythm_playback_state SET is_playing = FALSE, is_paused = FALSE, position_seconds = %s, track_uid = %s, play_session_key = %s WHERE guild_id = %s AND bot_name = 'rhythm'",
...
async def on_wavelink_track_exception(payload):
    try:
        ...
        logger.exception("[rhythm] Failed to handle Wavelink track exception without leaking queue state.")
...
async def on_wavelink_track_stuck(payload):
    try:
        ...
        logger.exception("[rhythm] Failed to handle Wavelink stuck track without leaking queue state.")
...
async def get_home_channel(guild):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT home_vc_id FROM rhythm_bot_home_channels WHERE guild_id = %s AND bot_name = 'rhythm'", (guild.id,))
...
async def send_or_update_status_message(guild, embed):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("CREATE TABLE IF NOT EXISTS rhythm_status_messages (guild_id BIGINT, bot_name VARCHAR(50), feedback_channel_id BIGINT, message_id BIGINT, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, PRIMARY KEY (guild_id, bot_name))")
                await cur.execute("SELECT feedback_channel_id FROM rhythm_guild_settings WHERE guild_id = %s", (guild.id,))
                res = await cur.fetchone()
                ...
                if cached_message_id:
                    ...
                    await cur.execute("SELECT message_id FROM rhythm_status_messages WHERE guild_id = %s AND bot_name = 'rhythm' LIMIT 1", (guild.id,))
                ...
                await cur.execute("REPLACE INTO rhythm_status_messages (guild_id, bot_name, feedback_channel_id, message_id) VALUES (%s, 'rhythm', %s, %s)", (guild.id, channel.id, new_message_id))
...
async def is_dj(interaction: discord.Interaction, silent=False):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT dj_role_id, dj_only_mode FROM rhythm_guild_settings WHERE guild_id = %s", (interaction.guild.id,))
                res = await cur.fetchone()
                if res and res[1]:
            ...
            for _track_end_attempt in range(3):
                try:
                    async with DBPoolManager() as pool:
                        ...
                        async with pool.acquire() as conn:
                            async with conn.cursor() as cur:
                                loop_mode_coro = cur.execute("SELECT loop_mode FROM rhythm_guild_settings WHERE guild_id = %s", (guild.id,))
                                ...
                                if reason == "FINISHED":
                                    ...
                                    if loop_mode == 'queue':
                                        ...
                                        await insert_queue_front(cur, "rhythm_queue", guild.id, "rhythm", track_uri, track_title, original_requester, resolved_track_uid)
                                    else:
...
async def _prefetch_next_track_bg(guild_id: int, url: str, title: str) -> None:
    ...
    try:
        ...
        _rhythm_prefetch_cache[guild_id] = {
            'url': url, 'title': title, 'track': track,
            'expires_at': time.monotonic() + 300,
        }
        logger.debug("[rhythm] Prefetched next track for guild %s: %s", guild_id, title)
    except Exception:
        _rhythm_prefetch_cache.pop(guild_id, None)
...
async def _process_queue_inner(guild, channel_id, start_position=0, *, allow_recovery_restore=False):
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT volume, loop_mode, filter_mode, transition_mode, custom_speed, custom_pitch, custom_modifiers_left, stay_in_vc, fade_seconds, fade_curve, filter_stack FROM rhythm_guild_settings WHERE guild_id = %s", (guild.id,))
                res = await cur.fetchone()
                ...
                await cur.execute("SELECT id, video_url, title, requester_id, track_uid FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT 1", (guild.id,))
                ...
                if not next_song:
                    await cur.execute(
                        "UPDATE rhythm_playback_state SET channel_id = NULL, video_url = NULL, title = NULL, position_seconds = 0, is_playing = FALSE, is_paused = FALSE, play_session_key = NULL, track_uid = NULL WHERE guild_id = %s AND bot_name = 'rhythm'",
                        (guild.id,),
                    )
                    ...
                    if loop_mode != 'queue' and not allow_recovery_restore:
                        await cur.execute("DELETE FROM rhythm_queue_backup WHERE guild_id = %s AND bot_name = 'rhythm'", (guild.id,))
                ...
                await cur.execute("DELETE FROM rhythm_queue WHERE id = %s AND guild_id = %s AND bot_name = 'rhythm'", (song_id, guild.id))
                try:
                    # Use prefetch cache if available for this guild+url (eliminates Lavalink round-trip gap)
                    _cached = _rhythm_prefetch_cache.get(guild.id)
                    if _cached and _cached.get('url') == url and _cached.get('expires_at', 0) > time.monotonic():
                        track = _cached['track']
                        resolved_source = url
                        _rhythm_prefetch_cache.pop(guild.id, None)
                        logger.debug("[rhythm] Prefetch cache hit for guild %s: %s", guild.id, title)
                    else:
                    ...
                    if _is_direct_media_url(url):
                        playback_tracking.pop(guild.id, None)
                        await cur.execute(
                            "UPDATE rhythm_playback_state SET video_url = NULL, title = NULL, is_playing = FALSE, is_paused = FALSE, position_seconds = 0, play_session_key = NULL, track_uid = NULL WHERE guild_id = %s AND bot_name = 'rhythm'",
                            (guild.id,),
                        )
                ...
                try:
                    ...
                    if c_mod_left > 0:
                        ...
                        c_mod_left -= 1
                        await cur.execute("UPDATE rhythm_guild_settings SET custom_modifiers_left = %s WHERE guild_id = %s", (c_mod_left, guild.id))
                        if c_mod_left == 0: await cur.execute("UPDATE rhythm_guild_settings SET custom_speed = 1.0, custom_pitch = 1.0 WHERE guild_id = %s", (guild.id,))
                ...
                try:
                    await cur.execute(
                        "SELECT video_url, title FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT 1",
                        (guild.id,),
                    )
                ...
                if start_position <= 0:
                    await cur.execute("INSERT INTO rhythm_history (guild_id, video_url, title, requester_id) VALUES (%s, %s, %s, %s)", (guild.id, url, title, requester_id))
...
async def stop_playback(guild):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("UPDATE rhythm_playback_state SET channel_id = NULL, video_url = NULL, title = NULL, position_seconds = 0, is_playing = FALSE, is_paused = FALSE, play_session_key = NULL, track_uid = NULL WHERE guild_id = %s AND bot_name = 'rhythm'", (guild.id,))
                await cur.execute("DELETE FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm'", (guild.id,))
                await cur.execute("DELETE FROM rhythm_queue_backup WHERE guild_id = %s AND bot_name = 'rhythm'", (guild.id,))
                await cur.execute("UPDATE rhythm_voice_state SET desired_connected = FALSE, connected_channel_id = NULL, disconnected_at = CURRENT_TIMESTAMP WHERE guild_id = %s AND bot_name = 'rhythm'", (guild.id,))
            ...
            try:
                async with DBPoolManager() as pool:
                    async with pool.acquire() as conn:
                        async with conn.cursor() as cur:
                            await cur.execute(
                                "SELECT home_vc_id FROM rhythm_bot_home_channels WHERE guild_id = %s AND bot_name = 'rhythm' LIMIT 1",
                                (target_guild_id,),
                            )
        ...
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    await cur.execute(
                        "SELECT position_seconds, video_url, title, track_uid FROM rhythm_playback_state WHERE guild_id = %s AND bot_name = 'rhythm' AND (is_playing = TRUE OR is_paused = TRUE OR position_seconds > 0 OR video_url IS NOT NULL OR title IS NOT NULL) ORDER BY is_playing DESC, is_paused DESC, position_seconds DESC LIMIT 1",
                        (target_guild_id,),
                    )
    ...
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            await cur.execute(
                ...
                "FROM rhythm_playback_state WHERE guild_id = %s AND bot_name = 'rhythm' LIMIT 1",
                (guild_id,),
            )
...
async def _drsd_fetch_voice(pool, guild_id):
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            await cur.execute(
                "SELECT COALESCE(connected_channel_id, last_channel_id) FROM rhythm_voice_state WHERE guild_id = %s AND bot_name = 'rhythm' AND desired_connected = TRUE LIMIT 1",
                (guild_id,),
            )
            return await cur.fetchone()
...
async def _drsd_fetch_queue_count(pool, guild_id):
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            await cur.execute(
                "SELECT COUNT(*) FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm'",
                (guild_id,),
            )
            return await cur.fetchone()
...
async def _drsd_fetch_backup_count(pool, guild_id):
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            await cur.execute(
                "SELECT COUNT(*) FROM rhythm_queue_backup WHERE guild_id = %s AND bot_name = 'rhythm'",
                (guild_id,),
            )
            return await cur.fetchone()
...
async def bootstrap_recovery_states_from_db():
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                for table_name in ("rhythm_playback_state", "rhythm_queue", "rhythm_queue_backup", "rhythm_bot_home_channels"):
...
async def on_wavelink_node_ready(payload: wavelink.NodeReadyEventPayload):
    ...
    try:
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    await cur.execute("SELECT guild_id, channel_id, position_seconds, video_url, title FROM rhythm_playback_state WHERE bot_name = 'rhythm' AND (is_playing = TRUE OR is_paused = TRUE OR position_seconds > 0)")
        ...
        if not (tracked or guild_id in guild_states):
            ...
            try:
                async with DBPoolManager() as pool:
                    async with pool.acquire() as conn:
                        async with conn.cursor() as cur:
                            await cur.execute("SELECT stay_in_vc FROM rhythm_guild_settings WHERE guild_id = %s", (guild_id,))
    ...
    try:
        ...
        for guild, result in zip(guild_snapshot, reconcile_results):
            if isinstance(result, Exception):
                logger.exception("[rhythm] Metrics reconcile failed for guild %s", getattr(guild, 'id', None))
    except Exception as tx_error:
        logger.exception("[rhythm] Metrics heartbeat pre-pass failed.")
    try:
        await collect_and_persist_metrics()
    except Exception as tx_error:
        logger.exception("[rhythm] Metrics heartbeat failed.")
...
# --- SETTINGS COMMANDS ---
@bot.tree.command(name="rhythm_main_sethome", description="Save this bot's default voice or stage channel for join, autoplay, and recovery behavior.")
...
async def sethome(interaction: discord.Interaction, channel: discord.VoiceChannel | discord.StageChannel):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("REPLACE INTO rhythm_bot_home_channels (guild_id, bot_name, home_vc_id) VALUES (%s, %s, %s)", (interaction.guild.id, 'rhythm', channel.id))
...
@bot.tree.command(name="rhythm_main_setfeedback", description="Choose the text channel (or thread) for updates, queue actions, and recovery notices.")
@commands.has_permissions(administrator=True)
async def setfeedback(interaction: discord.Interaction, channel: discord.TextChannel | discord.Thread):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("UPDATE rhythm_guild_settings SET feedback_channel_id = %s WHERE guild_id = %s", (channel.id, interaction.guild.id))
    invalidate_feature_caches(interaction.guild.id)
...
@bot.tree.command(name="rhythm_main_djrole", description="Set the server DJ role that can manage restricted playback, queue, and settings commands.")
@commands.has_permissions(administrator=True)
async def djrole(interaction: discord.Interaction, role: discord.Role):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("UPDATE rhythm_guild_settings SET dj_role_id = %s WHERE guild_id = %s", (role.id, interaction.guild.id))
    invalidate_feature_caches(interaction.guild.id)
...
@bot.tree.command(name="rhythm_main_removedj", description="Clear the configured DJ role so only admins or open-access mode can control restricted commands.")
@commands.has_permissions(administrator=True)
async def removedj(interaction: discord.Interaction):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("UPDATE rhythm_guild_settings SET dj_role_id = NULL WHERE guild_id = %s", (interaction.guild.id,))
    invalidate_feature_caches(interaction.guild.id)
...
@bot.tree.command(name="rhythm_main_djmode", description="Enable or disable Strict DJ Mode so only admins and the DJ role can use control commands.")
@commands.has_permissions(administrator=True)
async def toggle_djmode(interaction: discord.Interaction):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT dj_only_mode FROM rhythm_guild_settings WHERE guild_id = %s", (interaction.guild.id,))
                res = await cur.fetchone()
                new_val = not res[0] if res else True
                await cur.execute("UPDATE rhythm_guild_settings SET dj_only_mode = %s WHERE guild_id = %s", (new_val, interaction.guild.id))
    invalidate_feature_caches(interaction.guild.id)
...
@bot.tree.command(name="rhythm_main_247", description="Keep the bot connected and ready in voice channels even after playback ends until you disable it.")
@commands.has_permissions(administrator=True)
async def toggle_247(interaction: discord.Interaction):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT stay_in_vc FROM rhythm_guild_settings WHERE guild_id = %s", (interaction.guild.id,))
                res = await cur.fetchone()
                new_val = not res[0] if res else True
                await cur.execute("UPDATE rhythm_guild_settings SET stay_in_vc = %s WHERE guild_id = %s", (new_val, interaction.guild.id))
    invalidate_feature_caches(interaction.guild.id)
    state = "ENABLED" if new_val else "DISABLED"
...
@bot.tree.command(name="rhythm_main_restart", description="Restart this bot instance immediately for maintenance or recovery. Administrator only.")
@commands.has_permissions(administrator=True)
...
@bot.tree.command(name="rhythm_main_play", description="Queue a track, URL, livestream, search result, or playlist and start playback if idle.")
...
async def play(interaction: discord.Interaction, search: str, source: str = "ytmsearch"):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT home_vc_id FROM rhythm_bot_home_channels WHERE guild_id = %s AND bot_name = 'rhythm'", (interaction.guild.id,))
                res = await cur.fetchone()
            ...
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    if queue_rows:
                        await cur.executemany(
                            "INSERT INTO rhythm_queue (guild_id, bot_name, video_url, title, requester_id, track_uid) VALUES (%s, %s, %s, %s, %s, %s)",
                            [(interaction.guild.id, 'rhythm', url, title, requester_id, track_uid) for url, title, requester_id, track_uid in queue_rows],
                        )
                        try:
                            ...
                            logger.debug("[rhythm] Bulk track intelligence queue write skipped.", exc_info=True)
                    if added_count > 1:
                    ...
                    await cur.execute("SELECT COUNT(*) FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm'", (interaction.guild.id,))
...
@bot.tree.command(name="rhythm_main_playnext", description="Queue one track to play next, ahead of the existing queue, without clearing current playback.")
async def playnext(interaction: discord.Interaction, search: str):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT id, video_url, title, requester_id, track_uid FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC", (interaction.guild.id,))
                existing_rows = await cur.fetchall()
                insert_data = [(interaction.guild.id, 'rhythm', track.uri, track.title, interaction.user.id, _new_track_uid())]
                insert_data.extend((
                ...
                try:
                    await cur.execute("START TRANSACTION")
                    await cur.execute("DELETE FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm'", (interaction.guild.id,))
                    await cur.executemany(
                        "INSERT INTO rhythm_queue (guild_id, bot_name, video_url, title, requester_id, track_uid) VALUES (%s, %s, %s, %s, %s, %s)",
                        insert_data,
                    )
                    ...
                    try:
                        ...
                        logger.debug("Suppressed exception", exc_info=True)
                        pass
                    logger.exception("[rhythm] playnext transaction failed; queue was rolled back instead of leaking tracks.")
                    raise
    vc = interaction.guild.voice_client
...
@bot.tree.command(name="rhythm_main_skip", description="Skip the current track and move playback to the next queued item or Auto-DJ recommendation.")
...
@bot.tree.command(name="rhythm_main_stop", description="Stop playback, clear the queue, remove recovery state, and reset the bot for this server.")
...
@bot.tree.command(name="rhythm_main_sleep", description="Stop playback and disconnect after N minutes. Use 0 to cancel an active timer.")
...
@bot.tree.command(name="rhythm_main_pause", description="Pause the current track without clearing the queue or playback position.")
...
@bot.tree.command(name="rhythm_main_resume", description="Resume the paused track from its current playback position.")
...
@bot.tree.command(name="rhythm_main_clear", description="Clear the upcoming queue, stop playback, and reset stored playback state for this server.")
async def clear(interaction: discord.Interaction):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("DELETE FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm'", (interaction.guild.id,))
                await cur.execute("DELETE FROM rhythm_queue_backup WHERE guild_id = %s AND bot_name = 'rhythm'", (interaction.guild.id,))
                await cur.execute("UPDATE rhythm_playback_state SET channel_id = NULL, video_url = NULL, title = NULL, position_seconds = 0, is_playing = FALSE, is_paused = FALSE, play_session_key = NULL, track_uid = NULL WHERE guild_id = %s AND bot_name = 'rhythm'", (interaction.guild.id,))
                await cur.execute("UPDATE rhythm_voice_state SET desired_connected = FALSE, connected_channel_id = NULL, disconnected_at = CURRENT_TIMESTAMP WHERE guild_id = %s AND bot_name = 'rhythm'", (interaction.guild.id,))
    try:
        await bot.change_presence(status=discord.Status.online)
    except Exception as tx_error:
        logger.debug("Suppressed exception", exc_info=True)
...
@bot.tree.command(name="rhythm_main_join", description="Force the bot to join your current voice channel, or its configured home channel if one is saved.")
...
@bot.tree.command(name="rhythm_main_leave", description="Disconnect the bot from voice and clear any pending recovery handoff for this server.")
...
@bot.tree.command(name="rhythm_main_queue", description="Show the current queue with paging, requester names, and track positions")
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute("SELECT title, requester_id FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT %s OFFSET %s", (interaction.guild.id, per_page, offset))
...
@bot.tree.command(name="rhythm_main_shuffle", description="Smart-shuffle the upcoming queue while keeping duplicate tracks separated when possible.")
    ...
    try:
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    await cur.execute(
                        "SELECT video_url, title FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT 200",
                        (guild.id,),
                    )
...
@bot.tree.command(name="rhythm_main_remove", description="Remove a queued track by its queue number so it will not play later.")
...
async def remove(interaction: discord.Interaction, index: int):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT id, video_url, title, track_uid FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT 1 OFFSET %s", (interaction.guild.id, index-1))
                row = await cur.fetchone()
                if row:
                    await cur.execute("DELETE FROM rhythm_queue WHERE id = %s AND guild_id = %s AND bot_name = 'rhythm'", (row[0], interaction.guild.id))
...
@bot.tree.command(name="rhythm_main_skipto", description="Drop everything before a chosen queue position and jump playback forward to that track.")
@app_commands.autocomplete(index=_queue_index_autocomplete)
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute("SELECT video_url, title, track_uid FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT %s", (interaction.guild.id, skip_count))
                rows = list(await cur.fetchall() or [])
                if rows:
                    try:
                        await cur.execute("START TRANSACTION")
                        await cur.execute("DELETE FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT %s", (interaction.guild.id, skip_count))
                        ...
                        try:
                            ...
                            logger.debug("Suppressed exception", exc_info=True)
                            pass
                        logger.exception("[rhythm] skipto transaction failed; live/backup queues rolled back instead of drifting.")
                        raise
...
@bot.tree.command(name="rhythm_main_move", description="Move a queued track from one queue slot to another without rebuilding the entire session manually.")
                ...
                async with conn.cursor() as cur:
                    ...
                    try:
                        ...
                        try:
                            ...
                            logger.debug("Suppressed exception", exc_info=True)
                            pass
                        logger.exception("[rhythm] move transaction failed; queue was rolled back instead of leaking tracks.")
                        raise
...
@bot.tree.command(name="rhythm_main_bump", description="Move a queued track to the front so it plays next")
@app_commands.autocomplete(index=_queue_index_autocomplete)
async def bump(interaction: discord.Interaction, index: int):
    ...
    async with get_process_queue_lock(interaction.guild.id):
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    await cur.execute("SELECT id, video_url, title, requester_id, track_uid FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT 1 OFFSET %s", (interaction.guild.id, index-1))
                    row = await cur.fetchone()
                    if not row:
                        return await interaction.response.send_message("Invalid index.", ephemeral=True)
                    await cur.execute("DELETE FROM rhythm_queue WHERE id = %s AND guild_id = %s AND bot_name = 'rhythm'", (row[0], interaction.guild.id))
                    await insert_queue_front(cur, "rhythm_queue", interaction.guild.id, "rhythm", row[1], row[2], row[3], _row_track_uid(row, 4))
                    await snapshot_queue_backup(cur, interaction.guild.id)
...
@bot.tree.command(name="rhythm_main_clearmine", description="Remove your own queued songs without touching other listeners' tracks")
async def clearmine(interaction: discord.Interaction):
    await interaction.response.defer(ephemeral=True)
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT COUNT(*) FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' AND requester_id = %s", (interaction.guild.id, interaction.user.id))
                row = await cur.fetchone()
                removed = row[0] if row else 0
                if removed:
                    await cur.execute("DELETE FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' AND requester_id = %s", (interaction.guild.id, interaction.user.id))
                    # FIX 10: Mirror deletion in the backup queue
                    await cur.execute("DELETE FROM rhythm_queue_backup WHERE guild_id = %s AND bot_name = 'rhythm' AND requester_id = %s", (interaction.guild.id, interaction.user.id))
...
@bot.tree.command(name="rhythm_main_voteskip", description="Start or join a vote skip when no DJ is around to skip directly")
...
@bot.tree.command(name="rhythm_main_upvote", description="Upvote a queued track; enough listener upvotes bump it to play next.")
...
@bot.tree.command(name="rhythm_main_downvote", description="Downvote a queued track; enough listener downvotes removes it from the queue.")
...
@bot.tree.command(name="rhythm_main_radio", description="Queue several tracks matching a mood or vibe, e.g. 'chill lofi' or 'workout hype'.")
...
@bot.tree.command(name="rhythm_main_autodj", description="Enable or disable smarter Auto-DJ recommendations when the queue runs dry")
...
@bot.tree.command(name="rhythm_main_settings", description="Show the saved playback, DJ, queue, and recovery settings for this server")
...
@bot.tree.command(name="rhythm_main_playlists", description="List your saved personal playlists and how many tracks each one contains")
async def playlists(interaction: discord.Interaction):
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT playlist_name, COUNT(*) FROM rhythm_user_playlists WHERE user_id = %s GROUP BY playlist_name ORDER BY playlist_name ASC", (interaction.user.id,))
                rows = await cur.fetchall()
    if not rows:
...
@bot.tree.command(name="rhythm_main_deleteplaylist", description="Delete one of your saved personal playlists by name")
async def deleteplaylist(interaction: discord.Interaction, name: str):
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT COUNT(*) FROM rhythm_user_playlists WHERE user_id = %s AND playlist_name = %s", (interaction.user.id, name))
                row = await cur.fetchone()
                count = row[0] if row else 0
                if count:
                    await cur.execute("DELETE FROM rhythm_user_playlists WHERE user_id = %s AND playlist_name = %s", (interaction.user.id, name))
    if not count:
...
# --- PLAYLISTS & HISTORY ---
@bot.tree.command(name="rhythm_main_savequeue", description="Save the current queue to one of your personal playlists so you can load it again later.")
async def savequeue(interaction: discord.Interaction, name: str):
    await interaction.response.defer(ephemeral=True)
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT video_url, title FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC", (interaction.guild.id,))
                rows = await cur.fetchall()
                if not rows:
                ...
                await cur.executemany(
                    "INSERT INTO rhythm_user_playlists (user_id, playlist_name, video_url, title) VALUES (%s, %s, %s, %s)",
                    insert_data,
                )
...
@bot.tree.command(name="rhythm_main_loadqueue", description="Load one of your saved personal playlists into the active queue and start playback if needed.")
async def loadqueue(interaction: discord.Interaction, name: str):
    await interaction.response.defer(ephemeral=True)
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT video_url, title FROM rhythm_user_playlists WHERE user_id = %s AND playlist_name = %s", (interaction.user.id, name))
                rows = await cur.fetchall()
                if not rows:
...
@bot.tree.command(name="rhythm_main_leaderboard", description="Show the most played tracks from this server based on stored playback history.")
async def leaderboard(interaction: discord.Interaction):
    await interaction.response.defer(ephemeral=True)
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT title, COUNT(*) as plays FROM rhythm_history WHERE guild_id = %s GROUP BY title ORDER BY plays DESC LIMIT 10", (interaction.guild.id,))
                songs = await cur.fetchall()
    if not songs:
...
@bot.tree.command(name="rhythm_main_history", description="Show the most recent tracks played in this server from playback history.")
async def history(interaction: discord.Interaction):
    await interaction.response.defer(ephemeral=True)
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT title FROM rhythm_history WHERE guild_id = %s ORDER BY played_at DESC LIMIT 5", (interaction.guild.id,))
                songs = await cur.fetchall()
    if songs:
...
@bot.tree.command(name="rhythm_main_userhistory", description="Show the most recent tracks requested by a specific user in this server.")
async def userhistory(interaction: discord.Interaction, member: discord.Member):
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT id, title, video_url FROM rhythm_history WHERE guild_id = %s AND requester_id = %s ORDER BY played_at DESC LIMIT 10", (interaction.guild.id, member.id))
                songs = await cur.fetchall()
    ...
    embed.set_footer(text="Use /rhythm_main_steal <user> <number> to add one to the queue!")
    await interaction.response.send_message(embed=embed, ephemeral=True)

@bot.tree.command(name="rhythm_main_steal", description="Copy a track from a member's request history and add it back into the queue.")
async def steal(interaction: discord.Interaction, member: discord.Member, track_number: int):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT video_url, title FROM rhythm_history WHERE guild_id = %s AND requester_id = %s ORDER BY played_at DESC LIMIT 1 OFFSET %s", (interaction.guild.id, member.id, track_number - 1))
...
@bot.tree.command(name="rhythm_main_grab", description="Send yourself the currently playing track in a direct message for easy saving or sharing.")
...
@bot.tree.command(name="rhythm_main_like", description="Teach Auto-DJ to play more tracks like the current one for you.")
...
@bot.tree.command(name="rhythm_main_dislike", description="Teach Auto-DJ to avoid the current track for your future recommendations.")
...
@bot.tree.command(name="rhythm_main_recommend", description="Queue a smart recommendation learned from server taste, playlists, and listener feedback.")
...
@bot.tree.command(name="rhythm_main_taste", description="Show the saved taste profile Auto-DJ has learned for you or another member.")
async def taste(interaction: discord.Interaction, member: discord.Member = None):
    ...
    embed = discord.Embed(title=f"RHYTHM Taste Profile: {target.display_name}", description="\n".join(lines), color=discord.Color.blurple())
...
# --- MODIFIERS & FILTERS ---
@bot.tree.command(name="rhythm_main_volume", description="Set the playback volume for this server from 1 to 200 percent.")
async def volume(interaction: discord.Interaction, vol: int):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("INSERT INTO rhythm_guild_settings (guild_id, volume) VALUES (%s, %s) ON DUPLICATE KEY UPDATE volume = %s", (interaction.guild.id, vol, vol))
    if interaction.guild.voice_client:
...
@bot.tree.command(name="rhythm_main_loop", description="Choose whether playback loops nothing, the current song, or the full queue.")
async def loop_cmd(interaction: discord.Interaction, mode: str):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("INSERT INTO rhythm_guild_settings (guild_id, loop_mode) VALUES (%s, %s) ON DUPLICATE KEY UPDATE loop_mode = %s", (interaction.guild.id, mode, mode))
...
@bot.tree.command(name="rhythm_main_filter", description="Apply an audio filter such as nightcore, vaporwave, or bass boost to upcoming playback.")
...
async def filter_cmd(interaction: discord.Interaction, mode: str, stack: str = None):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                if stack is None:
                    await cur.execute("UPDATE rhythm_guild_settings SET filter_mode = %s, custom_speed = 1.0, custom_pitch = 1.0, custom_modifiers_left = 0 WHERE guild_id = %s", (mode, interaction.guild.id))
                else:
                    await cur.execute("UPDATE rhythm_guild_settings SET filter_mode = %s, filter_stack = %s, custom_speed = 1.0, custom_pitch = 1.0, custom_modifiers_left = 0 WHERE guild_id = %s", (mode, stack_value, interaction.guild.id))
    new_speed = 1.0
    if interaction.guild.voice_client:
        wav_filters = wavelink.Filters()
...
@bot.tree.command(name="rhythm_main_fade", description="Customize track fade transitions, let the bot pick smart fade timing, or enable BPM-matched mixing.")
...
async def toggle_fade(interaction: discord.Interaction, mode: str, seconds: float = 3.0, curve: str = "smooth"):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("UPDATE rhythm_guild_settings SET transition_mode = %s, fade_seconds = %s, fade_curve = %s WHERE guild_id = %s", (mode, seconds, curve, interaction.guild.id))
    if mode == "smart":
...
@bot.tree.command(name="rhythm_main_modify", description="Apply temporary custom speed and pitch modifiers to the next few tracks in the queue.")
...
async def modify_audio(interaction: discord.Interaction, speed: float = 1.0, pitch: float = 1.0, duration: int = 1):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT filter_mode FROM rhythm_guild_settings WHERE guild_id = %s", (interaction.guild.id,))
                res = await cur.fetchone()
                if res and res[0] != 'none': return await interaction.followup.send(embed=discord.Embed(description="❌ **Conflict:** Disable standard Filters via `/rhythm_main_filter none` first.", color=discord.Color.red()), ephemeral=True)
                if interaction.guild.id in playback_tracking:
                ...
                await cur.execute("UPDATE rhythm_guild_settings SET custom_speed = %s, custom_pitch = %s, custom_modifiers_left = %s WHERE guild_id = %s", (speed, pitch, duration, interaction.guild.id))
    if interaction.guild.voice_client:
        wav_filters = wavelink.Filters()
...
@bot.tree.command(name="rhythm_main_seek", description="Jump to an exact time in the current track using seconds from the start.")
...
@bot.tree.command(name="rhythm_main_forward", description="Jump forward within the current track by the number of seconds you provide.")
...
@bot.tree.command(name="rhythm_main_rewind", description="Jump backward within the current track by the number of seconds you provide.")
...
@bot.tree.command(name="rhythm_main_replay", description="Restart the current track from the beginning without changing the queue.")
...
async def build_panel_embed(guild):
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT volume, loop_mode FROM rhythm_guild_settings WHERE guild_id = %s", (guild.id,))
                row = await cur.fetchone()
    vol, loop_mode = row if row else (100, 'queue')
    embed = discord.Embed(title="🎛️ RHYTHM Music Control Panel", color=discord.Color.from_rgb(43, 45, 49))
    ...
    embed.set_footer(text="RHYTHM Main Music System")
    return embed


@bot.tree.command(name="rhythm_main_panel", description="Post an interactive control panel with playback, queue, and transport buttons.")
    ...
    class AdvancedPanel(discord.ui.View):
        ...
        async def _adjust_volume(self, i: discord.Interaction, delta: int):
            async with DBPoolManager() as pool:
                async with pool.acquire() as conn:
                    async with conn.cursor() as cur:
                        await cur.execute("SELECT volume FROM rhythm_guild_settings WHERE guild_id = %s", (i.guild.id,))
                        row = await cur.fetchone()
                        vol = max(1, min(200, (row[0] if row and row[0] else 100) + delta))
                        await cur.execute("INSERT INTO rhythm_guild_settings (guild_id, volume) VALUES (%s, %s) ON DUPLICATE KEY UPDATE volume = %s", (i.guild.id, vol, vol))
            if i.guild.voice_client:
        ...
        async def loop_toggle(self, i: discord.Interaction, _button: discord.ui.Button):
            ...
            async with DBPoolManager() as pool:
                async with pool.acquire() as conn:
                    async with conn.cursor() as cur:
                        await cur.execute("SELECT loop_mode FROM rhythm_guild_settings WHERE guild_id = %s", (i.guild.id,))
                        row = await cur.fetchone()
                        current_mode = row[0] if row and row[0] else 'queue'
                        next_mode = {'off': 'queue', 'queue': 'song', 'song': 'off'}.get(current_mode, 'queue')
                        await cur.execute("INSERT INTO rhythm_guild_settings (guild_id, loop_mode) VALUES (%s, %s) ON DUPLICATE KEY UPDATE loop_mode = %s", (i.guild.id, next_mode, next_mode))
            await self._refresh(i)
        ...
        async def vq(self, i: discord.Interaction, _button: discord.ui.Button):
            ...
            async with DBPoolManager() as pool:
                async with pool.acquire() as conn:
                    async with conn.cursor() as cur:
                        await cur.execute("SELECT title FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT 10", (i.guild.id,))
                        songs = await cur.fetchall()
...
@bot.tree.command(name="rhythm_main_lyrics", description="Show lyrics for the currently playing track, synced to the current position when available.")
...
@bot.tree.command(name="rhythm_main_nowplaying", description="Show the current track, progress bar, requester, and live playback status.")
...
@bot.tree.command(name="rhythm_main_ping", description="Show the bot websocket latency so you can quickly check responsiveness.")
...
@bot.tree.command(name="rhythm_main_uptime", description="Show how long this bot process has been running since its last startup.")
...
@bot.tree.command(name="rhythm_main_stats", description="Show quick bot statistics such as guild count and active player count.")
...
@bot.tree.command(name="rhythm_main_help", description="Show a categorized help menu for all RHYTHM music commands and utilities")
async def help_cmd(interaction: discord.Interaction):
    cmds = [c.name for c in bot.tree.get_commands() if c.name.startswith("rhythm_main_")]
...
async def search_playables(query):
    ...
    if _is_playlist_source(cleaned):
        try:
            ...
            logger.warning("[%s] yt-dlp playlist expansion failed for %s: %s", "rhythm", cleaned, exc)
    cache_key = cleaned.casefold()
...
async def _audio_cache_ids_still_needed_on_this_bot():
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                for queue_table in ("rhythm_queue", "rhythm_queue_backup"):
...
async def set_active_playlist(guild_id, playlist_url, known_track_count, requester_id, channel_id, playlist_entries=None):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("REPLACE INTO rhythm_active_playlists (guild_id, bot_name, playlist_url, known_track_count, requester_id, channel_id) VALUES (%s, 'rhythm', %s, %s, %s, %s)", (guild_id, playlist_url, known_track_count, requester_id, channel_id))
                if snapshot_rows:
                    await cur.execute("DELETE FROM rhythm_active_playlist_tracks WHERE guild_id = %s AND bot_name = 'rhythm'", (guild_id,))
                    await cur.executemany(
                        "INSERT INTO rhythm_active_playlist_tracks (guild_id, bot_name, playlist_url, position_idx, track_key, track_uid, video_url, title, requester_id) VALUES (%s, 'rhythm', %s, %s, %s, %s, %s, %s, %s)",
...
async def clear_active_playlist(guild_id):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("DELETE FROM rhythm_active_playlists WHERE guild_id = %s AND bot_name = 'rhythm'", (guild_id,))
                await cur.execute("DELETE FROM rhythm_active_playlist_tracks WHERE guild_id = %s AND bot_name = 'rhythm'", (guild_id,))
...
async def playlist_sync_loop():
    ...
    try:
        ...
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    await cur.execute("SELECT guild_id, playlist_url, known_track_count, requester_id, channel_id FROM rhythm_active_playlists WHERE bot_name = 'rhythm'")
                        ...
                        async with DBPoolManager() as pool:
                            async with pool.acquire() as conn:
                                async with conn.cursor() as cur:
                                    ...
                                    await cur.execute("SELECT track_key, track_uid, video_url, title, requester_id FROM rhythm_active_playlist_tracks WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY position_idx ASC", (guild_id,))
                                    ...
                                    purged_backup = 0
                                    if removed_counts:
                                        await cur.execute("SELECT id, video_url, title, track_uid FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC", (guild_id,))
                                        ...
                                        for live_id in live_delete_ids:
                                            await cur.execute("DELETE FROM rhythm_queue WHERE id = %s AND guild_id = %s AND bot_name = 'rhythm'", (live_id, guild_id))
                                            purged_live += 1

                                        await cur.execute("SELECT id, video_url, title, track_uid FROM rhythm_queue_backup WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC", (guild_id,))
                                        ...
                                        for backup_id in backup_delete_ids:
                                            await cur.execute("DELETE FROM rhythm_queue_backup WHERE id = %s AND guild_id = %s AND bot_name = 'rhythm'", (backup_id, guild_id))
                                            purged_backup += 1
                                    ...
                                    if added_rows:
                                        await cur.executemany(
                                            "INSERT INTO rhythm_queue (guild_id, bot_name, video_url, title, requester_id, track_uid) VALUES (%s, %s, %s, %s, %s, %s)",
                                            [(guild_id, 'rhythm', t_url, t_title, t_req, track_uid) for t_url, t_title, t_req, _key, track_uid in added_rows],
                                        )
                                        try:
                                            await bulk_record_tracks_queued(cur, guild_id, [(u, t, r) for u, t, r, _k, _uid in added_rows])
                                        except Exception as tx_error:
                                            logger.debug("[rhythm] Bulk playlist-sync intelligence write skipped.", exc_info=True)
                                    ...
                                    if current_rows:
                                        ...
                                        await cur.execute(
                                            "SELECT id, video_url, title FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC",
                                            (guild_id,)
                                        )
                                        ...
                                        for trim_id in trim_ids:
                                            await cur.execute("DELETE FROM rhythm_queue WHERE id = %s AND guild_id = %s AND bot_name = 'rhythm'", (trim_id, guild_id))
                                        trim_count = len(trim_ids)
                                        if trim_count > 0:
                                            logger.info("[rhythm] Playlist sync trimmed %s excess loop-mode queue copies for guild %s.", trim_count, guild_id)
                                    ...
                                    backup_content_trim = await dedupe_queue_table_by_track_key(cur, "rhythm_queue_backup", guild_id=guild_id, bot_name="rhythm")
                                    if backup_content_trim > 0:
                                        logger.info("[rhythm] Playlist sync removed %s content-duplicate backup row(s) for guild %s.", backup_content_trim, guild_id)
                                    ...
                                    await cur.execute(
                                        "SELECT video_url, title FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm'",
                                        (guild_id,),
                                    )
                                    _queued_keys = {
                                    ...
                                    if len(_queued_keys) < _refill_target:
                                        ...
                                        if refill_rows:
                                            await cur.executemany(
                                                "INSERT INTO rhythm_queue (guild_id, bot_name, video_url, title, requester_id, track_uid) VALUES (%s, %s, %s, %s, %s, %s)",
                                                [(guild_id, 'rhythm', t_url, t_title, t_req, track_uid) for t_url, t_title, t_req, _key, track_uid in refill_rows],
                                            )
                                            try:
                                                await bulk_record_tracks_queued(cur, guild_id, [(u, t, r) for u, t, r, _k, _uid in refill_rows])
                                            except Exception as tx_error:
                                                logger.debug("[rhythm] Bulk playlist-sync intelligence write skipped (refill).", exc_info=True)
                                    ...
                                    await cur.execute("UPDATE rhythm_active_playlists SET known_track_count = %s WHERE guild_id = %s AND bot_name = 'rhythm'", (current_count, guild_id))
                        break
...
async def aria_command_listener():
    try:
        ...
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    await cur.execute("SELECT guild_id, command, COALESCE(attempts, 0) AS attempts FROM rhythm_swarm_overrides WHERE bot_name = %s", ('rhythm',))
                ...
                async with DBPoolManager() as _restart_pool:
                    async with _restart_pool.acquire() as _restart_conn:
                        async with _restart_conn.cursor() as _restart_cur:
                            await _restart_cur.execute("DELETE FROM rhythm_swarm_overrides WHERE guild_id = %s AND bot_name = %s", (guild_id, 'rhythm'))
            ...
            if guild:
                if cmd == 'PAUSE' and vc and _player_is_playing(vc):
                    ...
                    if vc:
                        async with DBPoolManager() as _pool:
                            async with _pool.acquire() as _conn:
                                async with _conn.cursor() as _cur:
                                    await _cur.execute("SELECT filter_mode FROM rhythm_guild_settings WHERE guild_id = %s", (guild_id,))
                ...
                if executed:
                    try: await send_webhook_log(bot.user.name if bot.user else "Unknown Node", "🤖 Aria Override", f"Aria forcefully executed a **{cmd}** command in `{guild.name}`.", discord.Color.purple())
                        ...
                        logger.exception("[rhythm] Failed sending Aria override webhook for guild %s.", guild_id)
                else:
                    logger.info("[rhythm] Ignored Aria override %s for guild %s because the player state did not match.", cmd, guild_id)
            else:
                logger.warning("[rhythm] Received Aria override %s for unknown guild %s.", cmd, guild_id)
                ...
                async with _cleanup_pool.acquire() as _cleanup_conn:
                    async with _cleanup_conn.cursor() as _cleanup_cur:
                        if executed or attempts + 1 >= DIRECT_ORDER_MAX_ATTEMPTS or not guild:
                            await _cleanup_cur.execute("DELETE FROM rhythm_swarm_overrides WHERE guild_id = %s AND bot_name = %s", (guild_id, 'rhythm'))
                        else:
                            await _cleanup_cur.execute("UPDATE rhythm_swarm_overrides SET attempts = COALESCE(attempts, 0) + 1, last_error = %s WHERE guild_id = %s AND bot_name = %s", (f"state_mismatch:{cmd}", guild_id, 'rhythm'))
    except Exception as tx_error:
        logger.exception("Aria override listener failed for rhythm.")
    ...
    async with swarm_command_tables_lock:
        ...
        try:
            async with DBPoolManager() as pool:
                async with pool.acquire() as conn:
                    async with conn.cursor() as cur:
                        ...
                        for stmt in [
                            "ALTER TABLE rhythm_swarm_overrides ADD COLUMN bot_name VARCHAR(50) DEFAULT 'rhythm'",
                            "ALTER TABLE rhythm_swarm_overrides ADD COLUMN command VARCHAR(20) NULL",
                            "ALTER TABLE rhythm_swarm_direct_orders ADD COLUMN vc_id BIGINT NULL",
                            "ALTER TABLE rhythm_swarm_direct_orders ADD COLUMN text_channel_id BIGINT NULL",
                            "ALTER TABLE rhythm_swarm_direct_orders ADD COLUMN command VARCHAR(50) NULL",
                            "ALTER TABLE rhythm_swarm_direct_orders ADD COLUMN data TEXT NULL",
                            "ALTER TABLE rhythm_swarm_overrides ADD COLUMN attempts INT NOT NULL DEFAULT 0",
                            "ALTER TABLE rhythm_swarm_overrides ADD COLUMN last_error TEXT NULL",
                            "ALTER TABLE rhythm_swarm_direct_orders ADD COLUMN attempts INT NOT NULL DEFAULT 0",
                            "ALTER TABLE rhythm_swarm_direct_orders ADD COLUMN last_error TEXT NULL",
                            "ALTER TABLE rhythm_swarm_direct_orders ADD COLUMN claimed_at TIMESTAMP NULL",
                            "ALTER TABLE rhythm_swarm_direct_orders ADD COLUMN claim_token VARCHAR(128) NULL",
                            "ALTER TABLE rhythm_swarm_direct_orders ADD COLUMN created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP",
                            "ALTER TABLE rhythm_swarm_direct_orders ADD INDEX rhythm_direct_unclaimed_idx (bot_name, guild_id, claimed_at, id)",
                            "ALTER TABLE rhythm_swarm_direct_orders ADD INDEX rhythm_direct_claim_token_idx (claim_token)",
                            "ALTER TABLE rhythm_swarm_direct_orders ADD INDEX rhythm_direct_recent_idx (bot_name, guild_id, command, created_at)",
                        ]:
                            try:
            ...
            if now - _last_swarm_bridge_db_error_log_at >= SWARM_BRIDGE_DB_ERROR_LOG_INTERVAL_SECONDS:
                ...
                logger.warning("Swarm command tables unavailable for rhythm; bridge listeners will retry: %s", exc)
            return False
            ...
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    for raw_guild_id in active_guild_ids:
                        ...
                        await cur.execute("SELECT COUNT(*) FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm'", (guild.id,))
                        queue_count_row = await cur.fetchone()
                        queue_count = queue_count_row[0] if queue_count_row else 0
                        ...
                        # in rhythm_queue forever without being consumed (the "queue leak").
                        if queue_count > 0:
                            stuck_channel_id = (
                        ...
                        await cur.execute(
                            "SELECT is_playing, is_paused, position_seconds, video_url, title FROM rhythm_playback_state WHERE guild_id = %s AND bot_name = 'rhythm' LIMIT 1",
                            (guild.id,),
                        )
...
async def zombie_reaper_loop():
    ...
    for guild in list(bot.guilds):
        try:
            ...
            logger.exception("[rhythm] Zombie reaper failed to inspect voice state for guild %s.", getattr(guild, "id", "?"))
            continue
        if should_inspect:
            try:
                async with DBPoolManager() as pool:
                    async with pool.acquire() as conn:
                        async with conn.cursor() as cur:
                            ...
                            res = await cur.fetchone()
                            if res and res[0] == 0:
                                await cur.execute("SELECT COUNT(*) FROM rhythm_queue_backup WHERE guild_id = %s AND bot_name = 'rhythm'", (guild.id,))
                                backup_row = await cur.fetchone()
                                backup_count = backup_row[0] if backup_row else 0
                                if backup_count > 0:
                                    await cur.execute(
                                    ...
                                    await cur.execute("UPDATE rhythm_playback_state SET channel_id = NULL, video_url = NULL, title = NULL, position_seconds = 0, is_playing = FALSE, is_paused = FALSE, play_session_key = NULL, track_uid = NULL WHERE guild_id = %s AND bot_name = 'rhythm'", (guild.id,))
                                    continue
                                await cur.execute("SELECT stay_in_vc FROM rhythm_guild_settings WHERE guild_id = %s", (guild.id,))
                ...
                logger.exception("[rhythm] Zombie reaper failed for guild %s.", guild.id)
...
async def database_janitor_loop():
    try:
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    await cur.execute("DELETE FROM rhythm_history WHERE played_at < NOW() - INTERVAL 30 DAY")
                    ...
                    await cur.execute(
                        "DELETE FROM rhythm_smart_recommendations WHERE created_at < NOW() - INTERVAL 90 DAY"
                    )
                    await cur.execute(
                        "DELETE FROM rhythm_error_events WHERE created_at < NOW() - INTERVAL 30 DAY"
                    )
                    await cur.execute(
                        "DELETE FROM rhythm_track_intelligence WHERE last_played < NOW() - INTERVAL 365 DAY AND play_count <= 1 AND finish_count = 0"
                    )
                    await cur.execute(
                        "DELETE FROM rhythm_user_track_affinity WHERE last_requested < NOW() - INTERVAL 365 DAY AND play_count <= 1"
                    )
                    await cur.execute(
                        "DELETE FROM rhythm_track_cooccurrence WHERE updated_at < NOW() - INTERVAL 180 DAY"
                    )
                    await cur.execute(
                        "DELETE te FROM rhythm_track_embeddings te LEFT JOIN rhythm_track_intelligence ti ON ti.url_key = te.url_key "
...
async def queue_shuffle_maintenance_loop():
    ...
    try:
                ...
                async with conn.cursor() as cur:
                    for guild in list(bot.guilds):
                        ...
                        if not vc or not _voice_client_connected(vc) or not _has_human_listeners(vc):
                            continue
                        await cur.execute("SELECT loop_mode FROM rhythm_guild_settings WHERE guild_id = %s", (guild.id,))
                        row = await cur.fetchone()
        ...
        logger.warning("[rhythm] Scheduled queue reshuffle failed: %s", exc)
...
async def direct_order_listener():
    ...
    try:
                ...
                async with DBPoolManager() as pool:
                    async with pool.acquire() as conn:
                        async with conn.cursor(aiomysql.DictCursor) as cur:
                            ...
                            await cur.execute("DELETE FROM rhythm_swarm_direct_orders WHERE bot_name = %s AND created_at < NOW() - INTERVAL %s SECOND", ('rhythm', DIRECT_ORDER_STALE_SECONDS))
                            await cur.execute(
                                """
                                UPDATE rhythm_swarm_direct_orders
                                ...
                                LIMIT %s
                                """,
                                (current_claim_token, 'rhythm', DIRECT_ORDER_MAX_ATTEMPTS, DIRECT_ORDER_CLAIM_TIMEOUT_SECONDS, DIRECT_ORDER_FETCH_LIMIT),
                            )
                            if int(getattr(cur, "rowcount", 0) or 0) > 0:
                                await cur.execute(
                                    """
                                    SELECT id, bot_name, guild_id, vc_id, text_channel_id, command, data, COALESCE(attempts, 0) AS attempts
                                    FROM rhythm_swarm_direct_orders
                                    WHERE claim_token = %s
                                    ORDER BY id ASC
                                    LIMIT %s
                                    """,
            ...
            if guild:
                ...
                if cmd == 'PLAY':
                    ...
                    if vc_target:
                        ...
                        logger.warning("[rhythm] Dropped direct PLAY order %s for guild %s because no voice channel was resolved.", oid, guild.id)
                    ...
                    if vc and (_player_is_playing(vc) or _player_is_paused(vc)):
                        try:
                            ...
                            logger.warning("[rhythm] Direct SEEK failed in guild %s at %ss: %s", guild.id, position_seconds if 'position_seconds' in dir() else '?', _seek_err)
                    else:
                        logger.info("[rhythm] Direct SEEK ignored — no active player in guild %s.", guild.id)

                else:
                    logger.warning(f"[rhythm] Unsupported direct order {cmd!r} for guild {guild.id}.")
                ...
                if executed:
                    try:
                        ...
                        logger.exception("[rhythm] Failed sending direct order webhook for guild %s.", guild.id)
                else:
                    logger.info("[rhythm] Direct order %s in guild %s completed without state changes.", cmd, guild.id)
            else:
                logger.warning("[rhythm] Received direct order %s for unknown guild %s.", cmd, order['guild_id'])
                        ...
                        async with _fin_pool.acquire() as _fin_conn:
                            async with _fin_conn.cursor() as _fin_cur:
                                try:
                                    ...
                                    if executed or attempts + 1 >= DIRECT_ORDER_MAX_ATTEMPTS or not guild:
                                        await _fin_cur.execute("DELETE FROM rhythm_swarm_direct_orders WHERE id = %s", (oid,))
                                    else:
                                        await _fin_cur.execute("UPDATE rhythm_swarm_direct_orders SET attempts = COALESCE(attempts, 0) + 1, last_error = %s, claimed_at = DATE_SUB(NOW(), INTERVAL %s SECOND), claim_token = NULL WHERE id = %s", (f"unexecuted:{cmd}", DIRECT_ORDER_RETRY_BACKDATE_SECONDS, oid))
        ...
        logger.exception("Direct order listener failed for rhythm.")
...
class SwarmIntelligence(commands.Cog):
    ...
    async def watchdog(self):
        try:
            ...
            logger.exception("[rhythm] Watchdog tick failed.")
...
async def queue_integrity_check_loop():
    ...
    was dequeued (DELETE FROM rhythm_queue) at play-start but the bot crashed or
    disconnected before it finished, so it vanished from both the live queue and the
    player, yet the backup still has a copy.

    For each guild where rhythm_playback_state reports a track was playing but
    rhythm_queue is now empty and the player is not active, this loop locates that
    specific track in rhythm_queue_backup and re-inserts it at the front of the live
    ...
    try:
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    ...
                    await cur.execute(
                        """
                        SELECT guild_id FROM rhythm_queue WHERE bot_name = %s
                        UNION SELECT guild_id FROM rhythm_queue_backup WHERE bot_name = %s
                        UNION SELECT guild_id FROM rhythm_playback_state WHERE bot_name = %s
                        """,
                    ...
                    for parity_row in parity_rows:
                        ...
                        await cur.execute(
                            "SELECT "
                            "(SELECT COUNT(*) FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm') AS live_cnt, "
                            "(SELECT COUNT(*) FROM rhythm_queue_backup WHERE guild_id = %s AND bot_name = 'rhythm') AS backup_cnt",
                            (guild_id, guild_id),
                        )
                        ...
                        if should_defer_automatic_queue_recovery(guild_id):
                            continue
                        await cur.execute(
                            "SELECT channel_id FROM rhythm_playback_state WHERE guild_id = %s AND bot_name = %s LIMIT 1",
                            (guild_id, "rhythm"),
                        )
                        ...
                        if not target_channel_id:
                            await cur.execute(
                                "SELECT COALESCE(connected_channel_id, last_channel_id) AS channel_id FROM rhythm_voice_state WHERE guild_id = %s AND bot_name = %s LIMIT 1",
                                (guild_id, "rhythm"),
                            )
                            channel_row = await cur.fetchone()
                            target_channel_id = _row_value(channel_row, "channel_id", _row_value(channel_row, 0))
                    ...
                    for row in candidates:
                        ...
                        await cur.execute(
                            "SELECT video_url, title, requester_id, track_uid FROM rhythm_queue_backup "
                            "WHERE guild_id = %s AND bot_name = 'rhythm' "
        ...
        logger.exception("[rhythm] Queue integrity check loop failed.")
