...
async def reconcile_runtime_playback_state(guild):
    ...
    actual_playing = bool(vc and getattr(vc, "playing", False))
    actual_paused = bool(vc and getattr(vc, "paused", False))
            ...
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    for g in guilds:
                        ...
                        player_playing = bool(vc and getattr(vc, "playing", False))
                        player_paused = bool(vc and getattr(vc, "paused", False))
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                if not next_song:
                    await cur.execute("UPDATE rhythm_playback_state SET is_playing = FALSE, is_paused = FALSE WHERE guild_id = %s AND bot_name = 'rhythm'", (guild.id,))
...
async def clear(interaction: discord.Interaction):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("DELETE FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm'", (interaction.guild.id,))
                await cur.execute("UPDATE rhythm_playback_state SET is_playing = FALSE, is_paused = FALSE, position_seconds = 0 WHERE guild_id = %s AND bot_name = 'rhythm'", (interaction.guild.id,))
    await bot.change_presence(status=discord.Status.online)
...
async def remove(interaction: discord.Interaction, index: int):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT id FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT 1 OFFSET %s", (interaction.guild.id, index-1))
                row = await cur.fetchone()
                if row:
...
async def skipto(interaction: discord.Interaction, index: int):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT id FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT %s", (interaction.guild.id, index-1))
                rows = await cur.fetchall()
                for r in rows: await cur.execute("DELETE FROM rhythm_queue WHERE id = %s AND guild_id = %s AND bot_name = 'rhythm'", (r[0], interaction.guild.id))
...
async def aria_command_listener():
    try:
        ...
        for row in commands:
            ...
            if cmd == 'RESTART':
                logger.warning("[rhythm] Restart override received; exiting for container supervisor restart.")
                async with DBPoolManager() as pool:
                    async with pool.acquire() as conn:
                        async with conn.cursor() as cur:
            ...
            if guild:
                ...
                if cmd == 'PLAY':
                    ...
                    if recover_channel_id:
                        ...
                        logger.warning("[rhythm] Could not resolve RECOVER target for guild %s.", guild.id)
