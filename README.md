...
![Rhythm bot icon](assets/bot-icon.png)

Rhythm is one node in the 13-bot Discord music fleet. Built for servers that live and die by the beat. It owns its queue tables, reports playback state to the shared swarm database, and is controlled directly from Discord, Aria, and SwarmPanel.

## Identity

- Discord display name: **GWS Music Bot (Rhythm)**
- Internal node name: `rhythm`
- Command namespace: `/rhythm_main_*`
...
All commands are namespaced as `/rhythm_main_<command>`.
...
- Node: `rhythm`
- Database schema: `discord_music_rhythm`
