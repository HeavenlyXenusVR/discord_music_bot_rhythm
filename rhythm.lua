--[[
  RHYTHM — GWS Music Bot (Rhythm)
  LuaJIT port of rhythm.py, node `rhythm` of the 13-bot swarm.

  "Servers that live and die by the beat." Rhythm owns its queue tables,
  reports playback state to the shared swarm database, and is controlled
  directly from Discord (and, in the Python era, from Aria/SwarmPanel).
  This port keeps the command surface (/rhythm_main_*), the Postgres schema,
  and the flavor text/emoji identity of the original bot. It intentionally
  sheds the Python version's huge amount of asyncio/aiomysql-races-era
  resilience plumbing (Redis cross-process locks, DAVE-protocol aiohttp
  monkeypatch, local ffmpeg/aubio audio cache + BPM/loudness analysis,
  YouTube Data API playlist fallback, Gemini embeddings) that existed to
  paper over discord.py/wavelink/aiomysql specifics that don't apply to
  this simpler, single-process LuaJIT + swarmlua stack. See the final
  report for the exact list of what was dropped and why.
]]
...
-- ===========================================================================
-- Env / configuration (mirrors rhythm.py's BOT_ENV_PREFIX = "RHYTHM")
-- ===========================================================================
...
if TOKEN == "" then
  io.stderr:write("[rhythm] " .. PREFIX .. "_DISCORD_TOKEN is not set; refusing to start.\n")
  os.exit(1)
end
...
local DB_CONFIG = {
  ...
  database = env(PREFIX .. "_DB_NAME", "discord_music_rhythm"),
}
...
if LAVALINK_PASSWORD == "" then
  io.stderr:write("[rhythm] Set " .. PREFIX .. "_LAVALINK_PASSWORD or LAVALINK_PASSWORD before starting rhythm.\n")
  os.exit(1)
end
...
local function http_get_json(url)
  ...
  local ok, status = https.request({
    url = url,
    method = "GET",
    headers = { ["User-Agent"] = "swarmlua-rhythm/1.0" },
...
-- default per the README's guidance; rhythm needs neither message content
...
local function init_db()
  q([[INSERT INTO rhythm_guild_settings (guild_id, volume, loop_mode, filter_mode, transition_mode,
  ...
  q("DELETE FROM rhythm_guild_settings WHERE guild_id = %s", 0)
  ...
  q([[CREATE TABLE IF NOT EXISTS rhythm_active_playlists (
        guild_id BIGINT, bot_name VARCHAR(50) DEFAULT 'rhythm', playlist_url TEXT,
        known_track_count INT DEFAULT 0, requester_id BIGINT, channel_id BIGINT,
        PRIMARY KEY (guild_id, bot_name))]])
  q([[CREATE TABLE IF NOT EXISTS rhythm_active_playlist_tracks (
        guild_id BIGINT, bot_name VARCHAR(50) DEFAULT 'rhythm', playlist_url TEXT,
        position_idx INT DEFAULT 0, track_key VARCHAR(64), track_uid VARCHAR(32),
        video_url TEXT, title TEXT, requester_id BIGINT)]])
  q("CREATE INDEX IF NOT EXISTS rhythm_active_playlist_tracks_lookup_idx ON rhythm_active_playlist_tracks (guild_id, bot_name)")
...
local function get_settings(guild_id)
  ...
  local row = q1("SELECT * FROM rhythm_guild_settings WHERE guild_id = %s", guild_id)
...
local function get_home_channel_id(guild_id)
  local row = q1("SELECT home_vc_id FROM rhythm_bot_home_channels WHERE guild_id = %s AND bot_name = 'rhythm'", guild_id)
  return row and row.home_vc_id or nil
end

local function get_autodj_enabled(guild_id)
  local row = q1("SELECT auto_dj FROM rhythm_swarm_toggles WHERE guild_id = %s", guild_id)
  if row and row.auto_dj ~= nil then return row.auto_dj end
  return false
end

local function set_autodj_enabled(guild_id, enabled)
  q([[INSERT INTO rhythm_swarm_toggles (guild_id, auto_dj) VALUES (%s, %s)
      ON CONFLICT (guild_id) DO UPDATE SET auto_dj = EXCLUDED.auto_dj]], guild_id, enabled)
end

-- ===========================================================================
-- Discord embed / interaction reply helpers
-- (Bot:reply only does plain content; rhythm's whole personality is embeds,
...
local function current_position(guild_id)
  ...
  if data.paused then return math.floor(data.offset) end
  local elapsed = socket.gettime() - data.start_time
  return math.floor(data.offset + elapsed)
end
...
-- DB-backed search cache (rhythm_search_cache), 15 minute TTL -- mirrors the
-- Python bot's cache table without the extra in-process TTL cache layer.
local function search_cache_get(cache_key)
  local row = q1("SELECT resolved_uri, title FROM rhythm_search_cache WHERE cache_key = %s AND expires_at > NOW()", cache_key)
  if not row then return nil end
  return row.resolved_uri, row.title
end

local function search_cache_put(cache_key, uri, title)
  q([[INSERT INTO rhythm_search_cache (cache_key, resolved_uri, title, cached_at, expires_at)
...
-- ===========================================================================
-- Queue table helpers (rhythm_queue / rhythm_queue_backup)
-- ===========================================================================

local function queue_count(guild_id)
  local row = q1("SELECT COUNT(*)::int AS c FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm'", guild_id)
  return row and row.c or 0
end

local function snapshot_backup(guild_id)
  q("DELETE FROM rhythm_queue_backup WHERE guild_id = %s AND bot_name = 'rhythm'", guild_id)
  q([[INSERT INTO rhythm_queue_backup (guild_id, bot_name, video_url, title, requester_id, track_uid)
      SELECT guild_id, bot_name, video_url, title, requester_id, track_uid FROM rhythm_queue
      WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC]], guild_id)
end

local function enqueue_track(guild_id, url, title, requester_id, track_uid)
  track_uid = track_uid or new_uid()
  q([[INSERT INTO rhythm_queue (guild_id, bot_name, video_url, title, requester_id, track_uid)
      VALUES (%s, 'rhythm', %s, %s, %s, %s)]], guild_id, url, title, requester_id, track_uid)
  -- track_intelligence: queued_count bump (best-effort learning signal for Auto-DJ)
  local uk = track_key(url, title)
  q([[INSERT INTO rhythm_track_intelligence (guild_id, url_key, video_url, title, queued_count, play_count,
      ...
      ON CONFLICT (guild_id, url_key) DO UPDATE SET
        queued_count = rhythm_track_intelligence.queued_count + 1,
...
local function insert_queue_front(guild_id, url, title, requester_id, track_uid)
  ...
  local rows = q("SELECT id, video_url, title, requester_id, track_uid FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC", guild_id)
  q("DELETE FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm'", guild_id)
  q("INSERT INTO rhythm_queue (guild_id, bot_name, video_url, title, requester_id, track_uid) VALUES (%s, 'rhythm', %s, %s, %s, %s)",
...
local function delete_backup_track(guild_id, track_uid, video_url, title)
  if track_uid then
    q("DELETE FROM rhythm_queue_backup WHERE guild_id = %s AND bot_name = 'rhythm' AND track_uid = %s", guild_id, track_uid)
  elseif video_url then
    q([[DELETE FROM rhythm_queue_backup WHERE id IN (
          SELECT id FROM rhythm_queue_backup WHERE guild_id = %s AND bot_name = 'rhythm' AND video_url = %s LIMIT 1)]],
      guild_id, video_url)
  elseif title then
    q([[DELETE FROM rhythm_queue_backup WHERE id IN (
          SELECT id FROM rhythm_queue_backup WHERE guild_id = %s AND bot_name = 'rhythm' AND title = %s LIMIT 1)]],
      guild_id, title)
  end
end
...
local function restore_queue_from_backup(guild_id)
  local rows = q("SELECT video_url, title, requester_id, track_uid FROM rhythm_queue_backup WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC", guild_id)
  for _, r in ipairs(rows) do
    q("INSERT INTO rhythm_queue (guild_id, bot_name, video_url, title, requester_id, track_uid) VALUES (%s, 'rhythm', %s, %s, %s, %s)",
      guild_id, r.video_url, r.title, r.requester_id, col(r, "track_uid") or new_uid())
  end
...
-- (rhythm_active_playlists existed only as inert per-guild metadata,
...
local function clear_active_playlist(guild_id)
  q("DELETE FROM rhythm_active_playlists WHERE guild_id = %s AND bot_name = 'rhythm'", guild_id)
  q("DELETE FROM rhythm_active_playlist_tracks WHERE guild_id = %s AND bot_name = 'rhythm'", guild_id)
  playlist_missing_streak[guild_id] = nil
end

-- track_key() itself caps at 64 chars (matches rhythm_track_intelligence's
-- url_key VARCHAR(64)), but rhythm_active_playlist_tracks.track_key is a
...
local function set_active_playlist(guild_id, playlist_url, entries, requester_id, channel_id)
  ...
  q([[INSERT INTO rhythm_active_playlists (guild_id, bot_name, playlist_url, known_track_count, requester_id, channel_id)
  ...
  for idx, t in ipairs(entries) do
    ...
    q([[INSERT INTO rhythm_active_playlist_tracks (guild_id, bot_name, playlist_url, position_idx, track_key, track_uid, video_url, title, requester_id)
        VALUES (%s, 'rhythm', %s, %s, %s, %s, %s, %s, %s)]],
...
-- last-known snapshot (rhythm_active_playlist_tracks), enqueues additions,
...
local function playlist_sync_tick()
  local playlists = q("SELECT guild_id, playlist_url, known_track_count, requester_id, channel_id FROM rhythm_active_playlists WHERE bot_name = 'rhythm'") or {}
  for _, prow in ipairs(playlists) do
    ...
    local ok, err = pcall(function()
      ...
      local previous_rows = q("SELECT track_key, track_uid, video_url, title, requester_id FROM rhythm_active_playlist_tracks WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY position_idx ASC", gid) or {}
      ...
      if next(removed_counts) ~= nil then
        ...
        local live_rows = q("SELECT id, video_url, title FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC", gid) or {}
        for _, lr in ipairs(live_rows) do
          local k = playlist_track_key(lr.video_url, lr.title)
          if budget[k] and budget[k] > 0 then
            budget[k] = budget[k] - 1
            q("DELETE FROM rhythm_queue WHERE id = %s AND guild_id = %s AND bot_name = 'rhythm'", lr.id, gid)
            purged_live = purged_live + 1
          end
        end
        budget = {}
        for k, c in pairs(removed_counts) do budget[k] = c end
        local backup_rows = q("SELECT id, video_url, title FROM rhythm_queue_backup WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC", gid) or {}
        for _, br in ipairs(backup_rows) do
          local k = playlist_track_key(br.video_url, br.title)
          if budget[k] and budget[k] > 0 then
            budget[k] = budget[k] - 1
            q("DELETE FROM rhythm_queue_backup WHERE id = %s AND guild_id = %s AND bot_name = 'rhythm'", br.id, gid)
      ...
      local all_rows = q("SELECT id, video_url, title FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC", gid) or {}
      local seen = {}
      for _, qr in ipairs(all_rows) do
        local k = playlist_track_key(qr.video_url, qr.title)
        if current_keys[k] then
          if seen[k] then
            q("DELETE FROM rhythm_queue WHERE id = %s AND guild_id = %s AND bot_name = 'rhythm'", qr.id, gid)
      ...
      local refilled = 0
      local queued_rows = q("SELECT video_url, title FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm'", gid) or {}
      local queued_keys = {}
      local queued_n = 0
      ...
      q("DELETE FROM rhythm_active_playlist_tracks WHERE guild_id = %s AND bot_name = 'rhythm'", gid)
      local idx = 0
      for _, r in ipairs(current_rows) do
        q([[INSERT INTO rhythm_active_playlist_tracks (guild_id, bot_name, playlist_url, position_idx, track_key, track_uid, video_url, title, requester_id)
            VALUES (%s, 'rhythm', %s, %s, %s, %s, %s, %s, %s)]],
      ...
      q("UPDATE rhythm_active_playlists SET known_track_count = %s WHERE guild_id = %s AND bot_name = 'rhythm'", idx, gid)
    end)
    if not ok then
...
local function process_queue(guild_id, channel_id)
  ...
  local ok, err = pcall(function()
    ...
    if not next_row then
      if settings.loop_mode == "queue" then
        ...
        if restored > 0 then
          ...
          next_row = q1("SELECT id, video_url, title, requester_id, track_uid FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT 1", guild_id)
        end
      end
    end

    if not next_row then
      q([[UPDATE rhythm_playback_state SET channel_id = NULL, video_url = NULL, title = NULL,
          position_seconds = 0, is_playing = false, is_paused = false, play_session_key = NULL, track_uid = NULL
          WHERE guild_id = %s AND bot_name = 'rhythm']], guild_id)
    ...
    if settings.custom_modifiers_left and settings.custom_modifiers_left > 0 then
      ...
      q("UPDATE rhythm_guild_settings SET custom_modifiers_left = custom_modifiers_left - 1 WHERE guild_id = %s", guild_id)
      if settings.custom_modifiers_left - 1 <= 0 then
        q("UPDATE rhythm_guild_settings SET custom_speed = 1.0, custom_pitch = 1.0 WHERE guild_id = %s", guild_id)
      end
    else
    ...
    q([[INSERT INTO rhythm_playback_state (guild_id, bot_name, channel_id, video_url, position_seconds, is_playing, is_paused, title, play_session_key, track_uid)
        VALUES (%s, 'rhythm', %s, %s, 0, true, false, %s, %s, %s)
    ...
    q("INSERT INTO rhythm_history (guild_id, video_url, title, requester_id, played_at) VALUES (%s, %s, %s, %s, NOW())",
      guild_id, track.uri or url, track.title or title, requester_id)
    q([[INSERT INTO rhythm_track_intelligence (guild_id, url_key, video_url, title, queued_count, play_count, finish_count, skip_count, like_count, dislike_count, total_listen_seconds, last_requester_id, first_seen, last_played, updated_at)
        VALUES (%s, %s, %s, %s, 0, 1, 0, 0, 0, 0, 0, %s, NOW(), NOW(), NOW())
        ON CONFLICT (guild_id, url_key) DO UPDATE SET play_count = rhythm_track_intelligence.play_count + 1,
...
-- ===========================================================================
-- Smart Auto-DJ (simplified): scores candidates from rhythm_track_intelligence
-- and rhythm_user_track_affinity instead of the Python bot's Gemini-embedding
...
local function record_track_feedback(guild_id, user_id, url, title, liked)
  ...
  q([[INSERT INTO rhythm_track_intelligence (guild_id, url_key, video_url, title, queued_count, play_count, finish_count, skip_count, like_count, dislike_count, total_listen_seconds, first_seen, updated_at)
      VALUES (%s, %s, %s, %s, 0, 0, 0, 0, %s, %s, 0, NOW(), NOW())
      ON CONFLICT (guild_id, url_key) DO UPDATE SET like_count = rhythm_track_intelligence.like_count + %s,
        dislike_count = rhythm_track_intelligence.dislike_count + %s, updated_at = NOW()]],
    guild_id, uk, url, title, delta_like, delta_dislike, delta_like, delta_dislike)
  local score_delta = liked and 4.0 or -4.0
  q([[INSERT INTO rhythm_user_track_affinity (guild_id, user_id, url_key, video_url, title, queued_count, play_count, finish_count, skip_count, like_count, dislike_count, score, last_requested, updated_at)
      VALUES (%s, %s, %s, %s, %s, 0, 0, 0, 0, %s, %s, %s, NOW(), NOW())
      ON CONFLICT (guild_id, user_id, url_key) DO UPDATE SET
        like_count = rhythm_user_track_affinity.like_count + %s, dislike_count = rhythm_user_track_affinity.dislike_count + %s,
        score = rhythm_user_track_affinity.score + %s, updated_at = NOW()]],
...
local function pick_smart_recommendation(guild_id, listener_id)
  if listener_id then
    local row = q1([[SELECT video_url, title FROM rhythm_user_track_affinity
  ...
  local row = q1([[SELECT video_url, title FROM rhythm_track_intelligence
...
local function build_user_taste_summary(guild_id, user_id)
  local rows = q([[SELECT title, score FROM rhythm_user_track_affinity WHERE guild_id = %s AND user_id = %s ORDER BY score DESC LIMIT 8]], guild_id, user_id)
  local agg = q1([[SELECT COALESCE(SUM(play_count),0)::int AS played, COALESCE(SUM(finish_count),0)::int AS finished,
      ...
      COALESCE(SUM(skip_count),0)::int AS skipped FROM rhythm_user_track_affinity WHERE guild_id = %s AND user_id = %s]], guild_id, user_id)
  return rows, agg
end
...
bot.lavalink:on("event", function(msg)
  ...
  elseif msg.type == "TrackEndEvent" then
    ...
    if msg.reason == "finished" then
      ...
      if url then
        local uk = track_key(url, title)
        q("UPDATE rhythm_track_intelligence SET finish_count = finish_count + 1, total_listen_seconds = total_listen_seconds + %s, updated_at = NOW() WHERE guild_id = %s AND url_key = %s",
    ...
    elseif msg.reason == "loadFailed" or msg.reason == "cleanup" then
      ...
      if url then
        local uk = track_key(url, title)
        q("UPDATE rhythm_track_intelligence SET skip_count = skip_count + 1, updated_at = NOW() WHERE guild_id = %s AND url_key = %s", guild_id, uk)
      end
    end
...
local function cmdname(n) return "rhythm_main_" .. n end
...
bot:command(cmdname("sethome"), {
  ...
  q([[INSERT INTO rhythm_bot_home_channels (guild_id, bot_name, home_vc_id) VALUES (%s, 'rhythm', %s)
...
bot:command(cmdname("setfeedback"), {
  ...
  q("UPDATE rhythm_guild_settings SET feedback_channel_id = %s WHERE guild_id = %s", channel_id, guild_id_of(interaction))
...
bot:command(cmdname("djrole"), {
  ...
  ensure_guild_settings(guild_id_of(interaction))
  q("UPDATE rhythm_guild_settings SET dj_role_id = %s WHERE guild_id = %s", role_id, guild_id_of(interaction))
...
bot:command(cmdname("removedj"), {
  ...
  ensure_guild_settings(guild_id_of(interaction))
  q("UPDATE rhythm_guild_settings SET dj_role_id = NULL WHERE guild_id = %s", guild_id_of(interaction))
...
bot:command(cmdname("djmode"), {
  ...
  local row = q1("SELECT dj_only_mode FROM rhythm_guild_settings WHERE guild_id = %s", gid)
  local new_val = not (row and row.dj_only_mode)
  q("UPDATE rhythm_guild_settings SET dj_only_mode = %s WHERE guild_id = %s", new_val, gid)
...
bot:command(cmdname("247"), {
  ...
  local gid = guild_id_of(interaction)
  ensure_guild_settings(gid)
  local row = q1("SELECT stay_in_vc FROM rhythm_guild_settings WHERE guild_id = %s", gid)
  local new_val = not (row and row.stay_in_vc)
  q("UPDATE rhythm_guild_settings SET stay_in_vc = %s WHERE guild_id = %s", new_val, gid)
...
local function queue_index_choices(guild_id, needle)
  local rows = q("SELECT title FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT 200", guild_id)
...
bot:command(cmdname("play"), {
  ...
  if not channel_id then
    ...
    local row = q1("SELECT connected_channel_id FROM rhythm_voice_state WHERE guild_id = %s AND bot_name = 'rhythm'", gid)
    channel_id = row and row.connected_channel_id
  end
  if not channel_id then
    followup_embed(interaction, embed("Join a channel first or set a home channel with /rhythm_main_sethome.", { title = "\xE2\x9D\x8C Error", color = COLOR.red }))
    return
  end
...
function(_, interaction)
  ...
  stop_playback(gid)
  clear_active_playlist(gid)
  q("DELETE FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm'", gid)
  q("DELETE FROM rhythm_queue_backup WHERE guild_id = %s AND bot_name = 'rhythm'", gid)
  q([[UPDATE rhythm_playback_state SET channel_id = NULL, video_url = NULL, title = NULL, position_seconds = 0,
      is_playing = false, is_paused = false, play_session_key = NULL, track_uid = NULL WHERE guild_id = %s AND bot_name = 'rhythm']], gid)
...
function(_, interaction)
  ...
  q([[INSERT INTO rhythm_voice_state (guild_id, bot_name, connected_channel_id, desired_connected, last_seen_at, reconnect_attempts)
      VALUES (%s, 'rhythm', %s, true, NOW(), 0)
...
function(_, interaction)
  ...
  q([[UPDATE rhythm_voice_state SET desired_connected = false, connected_channel_id = NULL, disconnected_at = NOW() WHERE guild_id = %s AND bot_name = 'rhythm']], gid)
...
bot:command(cmdname("queue"), {
  ...
  local rows = q("SELECT title, requester_id FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT %s OFFSET %s", gid, per_page, offset)
  if #rows == 0 then
...
index_command("remove", "Remove a queued track by its queue number so it will not play later.", true, function(interaction, index)
  ...
  local row = q1("SELECT id, video_url, title, track_uid FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT 1 OFFSET %s", gid, index - 1)
  if not row then ack(interaction, "Invalid index.", true); return end
  q("DELETE FROM rhythm_queue WHERE id = %s AND guild_id = %s AND bot_name = 'rhythm'", row.id, gid)
...
index_command("skipto", "Drop everything before a chosen queue position and jump playback forward to that track.", true, function(interaction, index)
  ...
  local rows = q("SELECT video_url, title, track_uid FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT %s", gid, skip_n)
  if #rows > 0 then
    q("DELETE FROM rhythm_queue WHERE id IN (SELECT id FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT %s)", gid, skip_n)
...
bot:command(cmdname("move"), {
  ...
  local rows = q("SELECT id, video_url, title, requester_id, track_uid FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC", gid)
...
index_command("bump", "Move a queued track to the front so it plays next", true, function(interaction, index)
  local gid = guild_id_of(interaction)
  if index < 1 then ack(interaction, "Invalid index.", true); return end
  local row = q1("SELECT id, video_url, title, requester_id, track_uid FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT 1 OFFSET %s", gid, index - 1)
  if not row then ack(interaction, "Invalid index.", true); return end
  q("DELETE FROM rhythm_queue WHERE id = %s AND guild_id = %s AND bot_name = 'rhythm'", row.id, gid)
...
function(_, interaction)
  ...
  local row = q1("SELECT COUNT(*)::int AS c FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' AND requester_id = %s", gid, uid)
  local removed = row and row.c or 0
  if removed > 0 then
    q("DELETE FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' AND requester_id = %s", gid, uid)
    q("DELETE FROM rhythm_queue_backup WHERE guild_id = %s AND bot_name = 'rhythm' AND requester_id = %s", gid, uid)
  end
...
function(_, interaction)
  local uid = interaction.member and interaction.member.user and interaction.member.user.id
  local rows = q("SELECT playlist_name, COUNT(*)::int AS c FROM rhythm_user_playlists WHERE user_id = %s GROUP BY playlist_name ORDER BY playlist_name ASC", uid)
...
bot:command(cmdname("deleteplaylist"), {
  ...
  local row = q1("SELECT COUNT(*)::int AS c FROM rhythm_user_playlists WHERE user_id = %s AND playlist_name = %s", uid, name)
  local count = row and row.c or 0
  if count == 0 then ack(interaction, "That playlist was not found.", true); return end
  q("DELETE FROM rhythm_user_playlists WHERE user_id = %s AND playlist_name = %s", uid, name)
...
bot:command(cmdname("savequeue"), {
  ...
  local name = get_opt(interaction, "name")
  local rows = q("SELECT video_url, title FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC", gid)
  if #rows == 0 then followup_content(interaction, "Queue is empty!", true); return end
  for _, r in ipairs(rows) do
    q("INSERT INTO rhythm_user_playlists (user_id, playlist_name, video_url, title) VALUES (%s, %s, %s, %s)", uid, name, r.video_url, r.title)
  end
...
bot:command(cmdname("loadqueue"), {
  ...
  local gid, uid = guild_id_of(interaction), interaction.member and interaction.member.user and interaction.member.user.id
  local name = get_opt(interaction, "name")
  local rows = q("SELECT video_url, title FROM rhythm_user_playlists WHERE user_id = %s AND playlist_name = %s", uid, name)
...
function(_, interaction)
  defer(interaction, true)
  local gid = guild_id_of(interaction)
  local rows = q("SELECT title, COUNT(*)::int AS plays FROM rhythm_history WHERE guild_id = %s GROUP BY title ORDER BY plays DESC LIMIT 10", gid)
...
function(_, interaction)
  defer(interaction, true)
  local gid = guild_id_of(interaction)
  local rows = q("SELECT title FROM rhythm_history WHERE guild_id = %s ORDER BY played_at DESC LIMIT 5", gid)
...
bot:command(cmdname("userhistory"), {
  ...
  local rows = q("SELECT id, title, video_url FROM rhythm_history WHERE guild_id = %s AND requester_id = %s ORDER BY played_at DESC LIMIT 10", gid, member_id)
  ...
  ack_embed(interaction, embed(table.concat(lines, "\n"), { title = "\xF0\x9F\x8E\xA7 " .. name .. "'s Play History", color = COLOR.blue, footer = "Use /rhythm_main_steal <user> <number> to add one to the queue!" }), true)
end)
...
bot:command(cmdname("steal"), {
  ...
  local row = q1("SELECT video_url, title FROM rhythm_history WHERE guild_id = %s AND requester_id = %s ORDER BY played_at DESC LIMIT 1 OFFSET %s", gid, member_id, track_number - 1)
  if not row then
...
bot:command(cmdname("recommend"), {
  ...
  q([[INSERT INTO rhythm_smart_recommendations (guild_id, requester_id, seed_title, seed_url, query_text, chosen_url, chosen_title, reason, accepted, created_at)
...
bot:command(cmdname("taste"), {
  ...
  local e = embed(table.concat(lines, "\n"), { title = "RHYTHM Taste Profile: " .. name, color = COLOR.blurple })
  agg = agg or {}
...
bot:command(cmdname("volume"), {
  ...
  ensure_guild_settings(gid)
  q("UPDATE rhythm_guild_settings SET volume = %s WHERE guild_id = %s", vol, gid)
...
bot:command(cmdname("loop"), {
  ...
  ensure_guild_settings(gid)
  q("UPDATE rhythm_guild_settings SET loop_mode = %s WHERE guild_id = %s", mode, gid)
...
bot:command(cmdname("filter"), {
  ...
  if stack == nil then
    q("UPDATE rhythm_guild_settings SET filter_mode = %s, custom_speed = 1.0, custom_pitch = 1.0, custom_modifiers_left = 0 WHERE guild_id = %s", mode, gid)
  else
    q("UPDATE rhythm_guild_settings SET filter_mode = %s, filter_stack = %s, custom_speed = 1.0, custom_pitch = 1.0, custom_modifiers_left = 0 WHERE guild_id = %s", mode, stack_value, gid)
  end
  if playback[gid] then
    local filters = {}
...
bot:command(cmdname("fade"), {
  ...
  defer(interaction, true)
  ensure_guild_settings(gid)
  q("UPDATE rhythm_guild_settings SET transition_mode = %s, fade_seconds = %s, fade_curve = %s WHERE guild_id = %s", mode, seconds, curve, gid)
  local message, color
  if mode == "smart" then
...
bot:command(cmdname("modify"), {
  ...
  if s.filter_mode ~= "none" then
    followup_embed(interaction, embed("\xE2\x9D\x8C **Conflict:** Disable standard Filters via /rhythm_main_filter none first.", { color = COLOR.red }))
    return
  end
  q("UPDATE rhythm_guild_settings SET custom_speed = %s, custom_pitch = %s, custom_modifiers_left = %s WHERE guild_id = %s", speed, pitch, duration, gid)
  if playback[gid] then
...
local function build_panel_embed(gid)
  local s = get_settings(gid)
  local e = embed(nil, { title = "\xF0\x9F\x8E\x9B\xEF\xB8\x8F RHYTHM Music Control Panel", color = 0x2B2D31 })
  local data = playback[gid]
  e.fields = {}
  if data then
  ...
  e.footer = { text = "RHYTHM Main Music System" }
  return e
end

local PANEL_COMPONENTS = {
  { type = 1, components = {
    { type = 2, style = 1, label = "\xE2\x8F\xAF\xEF\xB8\x8F Play/Pause", custom_id = "rhythm:panel:playpause" },
    { type = 2, style = 4, label = "\xE2\x8F\xB9\xEF\xB8\x8F Stop", custom_id = "rhythm:panel:stop" },
    { type = 2, style = 2, label = "\xE2\x8F\xAD\xEF\xB8\x8F Skip", custom_id = "rhythm:panel:skip" },
  } },
  { type = 1, components = {
    { type = 2, style = 2, label = "\xE2\x8F\xAA -10s", custom_id = "rhythm:panel:rw" },
    { type = 2, style = 2, label = "\xE2\x8F\xA9 +10s", custom_id = "rhythm:panel:fw" },
    { type = 2, style = 2, label = "\xF0\x9F\x94\x89 Vol-", custom_id = "rhythm:panel:voldown" },
    { type = 2, style = 2, label = "\xF0\x9F\x94\x8A Vol+", custom_id = "rhythm:panel:volup" },
  } },
  { type = 1, components = {
    { type = 2, style = 2, label = "\xF0\x9F\x94\x81 Loop", custom_id = "rhythm:panel:looptoggle" },
    { type = 2, style = 3, label = "\xF0\x9F\x94\x80 Shuffle", custom_id = "rhythm:panel:shuffle" },
    { type = 2, style = 2, label = "\xF0\x9F\x93\x9C View Queue", custom_id = "rhythm:panel:queue" },
  } },
}
...
bot:command(cmdname("help"), { description = "Show a categorized help menu for all RHYTHM music commands and utilities" },
function(_, interaction)
...
bot.gateway:on("INTERACTION_CREATE", function(interaction)
  ...
  if interaction.type == 3 then -- MESSAGE_COMPONENT (panel buttons)
    ...
    local action = cid:match("^rhythm:panel:(.+)$")
    ...
    elseif action == "looptoggle" then
      ...
      q("UPDATE rhythm_guild_settings SET loop_mode = %s WHERE guild_id = %s", next_mode, gid)
      refresh()
    ...
    elseif action == "queue" then
      local rows = q("SELECT title FROM rhythm_queue WHERE guild_id = %s AND bot_name = 'rhythm' ORDER BY id ASC LIMIT 10", gid)
...
-- A much smaller cousin of rhythm.py's dozen tasks.loop()s -- this LuaJIT
...
-- rows to rhythm_swarm_direct_orders for source_url plays, fleet-supervisor
-- recovery, forced disconnects, and seek-to-position -- none of the simpler
-- overrides poller above covers these. Same claim-then-execute pattern.
local function handle_direct_order(order)
  ...
  if not ok then
    q("UPDATE rhythm_swarm_direct_orders SET attempts = attempts + 1, last_error = %s WHERE id = %s", tostring(err):sub(1, 500), order.id)
  else
    q("DELETE FROM rhythm_swarm_direct_orders WHERE id = %s", order.id)
    log_info("Aria executed direct order %s in guild %s.", cmd, guild_id)
  end
end
...
local function poll_direct_orders()
  ...
  for _, order in ipairs(rows) do
    q("UPDATE rhythm_swarm_direct_orders SET claimed_at = NOW() WHERE id = %s", order.id)
...
local function start_background_loops()
    ...
    while true do
      ...
      for gid, data in pairs(playback) do
        if not data.paused then
          local ok = pcall(function()
            q([[UPDATE rhythm_playback_state SET position_seconds = %s, last_checkpoint_at = NOW()
                WHERE guild_id = %s AND bot_name = 'rhythm']], current_position(gid), gid)
          end)
  ...
  copas.addthread(function()
    while true do
      copas.sleep(30)
      pcall(function()
        q([[INSERT INTO swarm_health (bot_name, last_pulse, status) VALUES ('rhythm', NOW(), 'online')
            ON CONFLICT (bot_name) DO UPDATE SET last_pulse = NOW(), status = 'online']])
        for gid, data in pairs(playback) do
          q([[INSERT INTO rhythm_metrics (guild_id, bot_name, voice_connected, player_connected, player_playing, player_paused,
                queue_count, backup_queue_count, is_playing_db, is_paused_db, position_seconds, duration_seconds, lavalink_ready, updated_at)
              VALUES (%s, 'rhythm', true, true, %s, %s, %s, 0, %s, %s, %s, %s, true, NOW())
...
log_info("RHYTHM booting -- %d commands registered, Lavalink at %s:%d", (function() local n=0; for _ in pairs(bot.commands) do n=n+1 end; return n end)(), LL_HOST, LL_PORT)
  ...
  copas.addthread(function()
    ...
    for _, row in ipairs(rows) do
      ...
      if channel_id then
        ...
        local was_playing = q1("SELECT is_playing FROM rhythm_playback_state WHERE guild_id = %s AND bot_name = 'rhythm'", gid)
